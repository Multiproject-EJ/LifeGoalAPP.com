...
Material-pass work has started but is **not accepted**. The current implementation adds independent deterministic roughness and bump maps to the five enamel families, Trial crystal, and Mirage; it also separates their clearcoat, metalness, specular intensity, environment response, and emissive hierarchy without adding draw calls or triangles. The normal full-spread evidence is `59-wide-chapter-4-material-pass.png`, with grazing and opposite-axis views in `60-wide-chapter-4-material-grazing.png` and `61-wide-chapter-4-material-neutral.png`. A dev-only `materialProof=1` camera detaches and frames the real active relief—not a synthetic swatch—to produce `66-chapter-4-material-proof.png`; that focused proof renders only the Chapter IV relief at **69 calls / 31.5k rendered triangles** and sampled **47 FPS**.

The material Tier-1 hard gate remains open. Its current whole-foreground five-cluster diagnostic omits the small teal and orange enamel regions even in the isolated relief, then emits every recipe result with `componentId: null`; the resulting maximum delta-E is therefore a harness failure, not accepted evidence. The pass stays at `tier1-diagnostics`: no AI review, `continue`, or pipeline sync has been recorded. The next correction must add genuine per-component visible-footprint crops to the deterministic material gate (or route these materials through the existing component-aware material comparator) before Chapter IV material acceptance. Do not bypass the failed gate by weakening the node palette or rewriting the sculpt spec to match background clusters.
