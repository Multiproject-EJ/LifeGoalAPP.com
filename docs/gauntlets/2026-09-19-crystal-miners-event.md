...
## Final user-requested polish before deployment

Added cavern-25 group merge: triple click/tap a tier, highlight matches, merge each pair once, retain odd leftovers and all other tiers. Canonical revision lock enforces the unlock and saves results with no currency spend. Also provides a visible equivalent button. Browser triple-click verified five tier-7 tools became two tier-8 tools plus one untouched tier-7, while existing tier-8/9 tools remained unchanged.

Replay compresses a long tail 2.25× only after the final chest the remaining simulation can reach, allowing the reveal first. It never changes physics or rewards. Added tests for no chest, later reachable chest, short tails and frame ordering.

User requests more expressive special blocks and a mobile game requiring no page scroll. Save format 7 adds rare ticket blocks (+1 exact mining drop), balloons (+gift), temporary pick spawners (bounded, same lane), ember blocks, distinct dust/ice/metal/fire/pop effects and ticket celebration flying to the counter. Existing tools, damage, prizes and funded drops migrate. Particles are bounded vector effects; no external assets. Mobile uses Workshop/Mine views, switches to Mine during drops, and returns to Workshop after results. Core controls measured inside the viewport with no overlay overflow at requested 390×844 and 320×568 sizes (browser's existing zoom affects CSS viewport dimensions). No changes to other games' layouts.

59 focused checks pass including special reward exactly-once accounting, bounded temporary spawns, full forty-cavern campaigns and all earlier integration checks. Both updated policies finish in 42 drops with retries on 38–39, one modeled island earning return at cavern 12 and unchanged exact main-game milestone rewards. First browser round failed without upgrades; a forge upgrade and second drop reached four chests and banked 25 dice. Continue/reload preserved cavern 2, forge 1, 506 ore, 58 drops and two digs. Main full local build stopped after more than forty minutes during changing scope; do not claim it passed. Release remains gated on GitHub's full TypeScript/Vite production build.

### Follow-up: difficulty without slow rebounds; guardian cannon

User clarified that difficulty should demand better equipment and rarer materials rather than longer waits, particularly at cavern 39. Removed the tier-scaled upward rebound: ordinary impacts now use a short drilling rebound, and broken rock releases tools downward. Existing HP, per-tool impact budgets and forge benefits remain meaningful. A fixed ten-tool tier-11/forge-5 cavern-39 benchmark falls from the old 40-second simulation cap to 17.78 seconds; it still fails with zero chests. Ten tier-7 tools finish failing in 12.57 seconds rather than 29.63. These are simulation times, not browser wall time (the replay has its own pacing).

Guardians now charge a visibly marked lane, fire a cannon that removes every active tool in that lane, reload and retarget. Target selection is deterministic and independent of occupancy or wallet, allowing misses. Higher guardians charge/reload faster. Killing the guardian cancels its attack, and terminal blasts remain visible for 600 ms. Destroyed falling tools do not delete permanent equipment. Attempts report boss shots and tools lost through the existing telemetry. Rare violet obsidian seams appear from cavern 21; schema 8 preserves saved investment and old broken/damaged seams during migration.

Regression coverage includes misses, full-lane removal, unaffected neighboring lanes, reload/retarget, death interruption, permanent inventory safety, migration and shorter late-level pacing. Isolated `course=cannon` and `course=pacing` development fixtures never touch the user's workshop. Browser final-guardian playtest reached two chests with the tier-9 fixture; automation initially missed brief charging/blast frames, so those captures were not counted as visual evidence.

Final follow-up checks: 64 tests passed; focused TypeScript checked 382 files with zero diagnostics; architecture guard passed with three existing allowlisted warnings; diff whitespace clean. Both complete canonical campaigns still finished in 42 drops. Phone guardian inspection verified the cannon silhouette and fitting controls; the level-39 weak rack failed at 16/28 m with 52 broken blocks and Try again returned to Workshop with tier-9 tools intact. Brief weapon-phase screenshots remain unverified because browser automation returned after the replay; the attack mechanics are covered by deterministic tests.

### Player feedback: focus, challenge, rotating themes and merge anticipation

User reported winning too consistently, asked for the grid to dominate preparation, informational details in `?`, six alternating cave environments, approximately one second of chest/reward celebration, admin/dev-only shaft inspection and mutual glow before a drag merge. Implemented those changes with a default player-focused workshop/drop flow, compact muted progress, a field-guide reward list, six authored palettes and cave motifs, a 1.1-second new-chest hold (also after Skip), and a two-tool merge-ready cue validated against actual merge eligibility. Inspection requires the launcher admin flag or explicit DEV inspector; local normal preview defaults to player mode.

Balance: the original upgraded policies cleared 40 caverns in 42 drops. An initial harder pass made final approaches take six or more attempts; reduced those final multipliers before release. Final canonical policies finish in 52 and 55 drops, with final approaches taking three/four attempts; gifts, upgrades, accumulated damage and all milestone payouts remain real canonical actions. Schema nine proportionally migrates existing damage and retains already opened paths and investment. No forced purchase or wallet-conditioned outcome was introduced.

Browser: normal player view contains no mine/shaft inspector buttons; requested 320×568 viewport has 631 CSS-pixel root height equal to scroll height, and Drop remains inside the viewport. Tap selection highlights nine matching tools; tapping a match changes only the pair (tier 7 + tier 7 → tier 8). The field guide opens and contains journey/group-merge details. A successful Frostglass drop returns via Continue to Emberforge preparation; explicit dev inspection shows the cave scenery. Brief chest/hover animation frames were not reliably captured because automation latency and a native-tool connection reset interrupted those observations; deterministic replay timing and merge eligibility tests cover the mechanics.

Final focus-release checks: 68 tests passed, focused TypeScript checked 383 source/declaration files with zero diagnostics, architecture guard passed with three existing allowlisted warnings, and diff whitespace was clean. Readiness guidance was raised with the terrain curve and its outdated tier-5 concentrated-fleet fixture corrected to tier 6. Ember and amethyst scenery were visually inspected at phone size. Temporary viewport override was reset and the owned QA tab closed; the player's original preview and save were not reset.
