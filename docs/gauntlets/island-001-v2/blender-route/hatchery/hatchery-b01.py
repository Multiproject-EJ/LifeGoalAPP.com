...
root['assetStatus']='draft-blockout-b01'; root['representation']='authored-blender-lofts-and-boolean-arcades'
...
upper=[(.55,-.81),(1.03,-.82),(1.17,-.64),(1.17,-.26),(.58,-.2),(.45,-.39)]
slab('upper-right-nursery-floor',upper,.987,1.035,trim,'wings',2)
...
basin=[(-.34,.5),(.83,.5),(.97,.66),(.83,.85),(-.27,.87),(-.45,.7)]
slab('middle-cascade-landing',basin,.445,.51,trim,edge=.012)
face=slab('middle-landing-open-arcade-support',[(-.29,.76),(.84,.76),(.84,.84),(-.29,.84)],.18,.46,stone,'terraces',edge=0)
for x in [-.08,.5]:
    cutter=arch_cutter('temporary-water-gallery',x,.8,(1,0),.40,.18,.245,.35)
...
slab('middle-pool-water',[(-.27,.56),(.74,.56),(.87,.67),(.73,.77),(-.22,.8),(-.35,.7)],.511,.527,water,edge=0)
low=[(-.17,.9),(.4,.91),(.43,1.08),(.26,1.18),(-.12,1.16),(-.26,1.05)]
slab('lower-shallow-pool-landing',low,.178,.239,trim,edge=.011)
slab('lower-pool-water',[(-.12,.94),(.34,.95),(.37,1.06),(.22,1.12),(-.09,1.10),(-.2,1.04)],.24,.255,water,edge=0)
...
waterfall('upper-cascade-water-sheet',.49,[(.997,.42),(.98,.52),(.56,.59),(.527,.68)])
waterfall('lower-cascade-water-sheet',.36,[(.529,.73),(.52,.83),(.28,.95),(.255,1.0)])
...
def petal_point(angle,height,t,u):
    radius=bezier(.52,.99,.83,.19,t)
    y=bezier(.996,1.14,height-.22,height,t)
    tangent=Vector((math.cos(angle),-math.sin(angle)));radial=Vector((math.sin(angle),math.cos(angle)))
    centre=radial*radius;half=.42*math.sin(math.pi*t)**.82
...
for number,degrees in enumerate(angles):
    a=math.radians(degrees);height={45:2.22,90:2.12,135:2.30,180:2.41}[abs(degrees)]
...
(OUT/'blockout-b01-geometry.json').write_text(json.dumps(report,indent=2)+'\n')
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'blockout-b01.blend'))
bpy.ops.export_scene.gltf(filepath=str(OUT/'blockout-b01.glb'),export_format='GLB',use_selection=False,export_yup=True,export_extras=True,export_materials='EXPORT',export_cameras=False,export_lights=False)
