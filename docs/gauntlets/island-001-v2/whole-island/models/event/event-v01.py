...
root=empty('ISLAND_001_V2_OBSERVATORY');root['id']='event';root['landmarkId']='event';root['logicalAxes']='Y-up;front+Z;ground0';root['assetStatus']='whole-island-v01-draft';root['sockets']={'lift':[.72,.15,1.07]}
...
glass=mat('observatory-clear-window-glass',(.62,.80,.79),.12,.02,alpha=.18)
...
DOME_Y=1.245;DOME_Z=-.25;RX=.94;RZ=.82;H=1.24

def dome_point(side,u,beta):
    cut=.24+.24*max(0,math.cos(beta))
...
(OUT/'event-v01-aperture-clearance-checks.json').write_text(json.dumps({'aperture':aperture,'liftClearanceRadius':.20,'liftClearanceHeight':[.20,1.15],'intrudingMeshes':near},indent=2)+'\n')
...
(OUT/'event-v01-support-checks.json').write_text(json.dumps(support_results,indent=2)+'\n')
...
(OUT/'event-v01-metadata.json').write_text(json.dumps(report,indent=2)+'\n')
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'event-v01.blend'))
bpy.ops.export_scene.gltf(filepath=str(OUT/'event-v01.glb'),export_format='GLB',export_yup=True,export_extras=True,export_materials='EXPORT',export_cameras=False,export_lights=False)
