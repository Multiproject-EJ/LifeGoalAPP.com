...
HERE=Path(__file__).resolve().parent;path=HERE/'habit-v01.glb';raw=path.read_bytes();assert raw[:4]==b'glTF'
...
parents={j:i for i,n in enumerate(g['nodes']) for j in n.get('children',[])};owners={'oak-trunk','oak-canopy','oak-galleries','oak-stairs'};records=[];mins=[math.inf]*3;maxs=[-math.inf]*3;radius=0
...
assert radius<=1.6 and maxs[1]<=3.7 and abs(mins[1])<1e-6
...
root=next(n for n in g['nodes'] if n.get('name')=='ISLAND_001_V2_HABIT_OAK');assert root['extras']['landmarkId']=='habit' and root['extras']['sockets']['lift']==[.72,.15,1.07]
...
(HERE/'habit-v01-export-checks.json').write_text(json.dumps(result,indent=2)+'\n')
