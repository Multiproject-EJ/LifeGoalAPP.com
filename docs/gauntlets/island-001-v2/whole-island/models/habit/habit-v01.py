...
root=empty('ISLAND_001_V2_HABIT_OAK');root['landmarkId']='habit';root['id']='habit';root['logicalAxes']='Y-up;front+Z;ground0';root['assetStatus']='whole-island-v01-draft';root['sockets']={'lift':[.72,.15,1.07]}
...
def mat(name,color,rough=.7,metal=0,emission=0,vertex=False):
    ...
    return m
bark=mat('oak-sculpted-bark',(.37,.24,.12),vertex=True)
...
def stair(name,points,from_y,to_y,width,level):
    ...
    for i in range(n):
        ...
        y=from_y+(to_y-from_y)*(i+1)/n;slab(name+'-tread-'+str(i),poly,y-.055,y,stone,'stairs',level,3)
    for side in [-1,1]:
        pts=[]
...
trunk_points=[(0,.17,-.22),(-.13,.77,-.23),(-.10,1.42,-.20),(-.23,2.02,-.19),(-.09,2.59,-.20),(.03,3.03,-.16)]
...
galleries=[(1,1.23,.46,.88,-112,145),(2,1.86,.39,.77,-8,143),(3,2.38,.31,.60,12,144)]
...
arch_panel('root-front-door',.08,.265,.17,.32,.62,1)
arch_panel('east-root-window',.58,.13,.25,.23,.39,2)
arch_panel('west-root-window',-.51,.17,.28,.20,.35,3)

# Low-cost organic leaf masses with fine individual leaf silhouettes, all sharing
# one vertex-colour material. The branch network remains visible below them.
def cloud(name,centre,scale,level,stage=4,flowers=False,detail=18):
    ...
    return mesh(name,verts,faces,leaf,'canopy',level,stage,colors)
...
for i,(level,p,s) in enumerate(clusters):
    ...
    for j in range(2):
        offset=(rng.uniform(-.11,.11),rng.uniform(-.06,.10),rng.uniform(-.11,.11));q=tuple(p[k]+offset[k] for k in range(3));ss=tuple(x*.52 for x in s);cloud('branch-leaflets-'+str(i)+'-'+str(j),q,ss,level,detail=12)
...
(OUT/'habit-v01-metadata.json').write_text(json.dumps(report,indent=2)+'\n')
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'habit-v01.blend'))
bpy.ops.export_scene.gltf(filepath=str(OUT/'habit-v01.glb'),export_format='GLB',export_yup=True,export_extras=True,export_materials='EXPORT',export_cameras=False,export_lights=False)
