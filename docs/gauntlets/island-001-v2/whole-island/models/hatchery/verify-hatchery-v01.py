...
HERE=Path(__file__).resolve().parent;path=HERE/'hatchery-v01.glb';raw=path.read_bytes();assert raw[:4]==b'glTF'
...
parents={j:i for i,n in enumerate(g['nodes']) for j in n.get('children',[])};owners={'hatchery-terraces','hatchery-wings','hatchery-conservatory','hatchery-egg'};records=[];mins=[math.inf]*3;maxs=[-math.inf]*3;radius=0
...
root=next(n for n in g['nodes'] if n.get('name')=='ISLAND_001_V2_LOTUS_HATCHERY');assert root['extras']['landmarkId']=='hatchery' and root['extras']['sockets']['lift']==[.72,.15,1.07]
...
(HERE/'hatchery-v01-export-checks.json').write_text(json.dumps(result,indent=2)+'\n')
