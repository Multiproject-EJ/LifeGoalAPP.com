...
export function enclosedPetal(theta: number, height: number, rows: number) {
  ...
  const sample = (t: number, u: number, thickness = 0) => {
    ...
    // A real doorway through the forward petals, including when all glass is opaque clay.
    const openingT = THREE.MathUtils.smoothstep(t, 0.66, 0.94);
    const openingHalfAngle = 0.43 * (1 - openingT);
    let leftAngle = theta - angularSpread;
    let rightAngle = theta + angularSpread;
    if (theta < Math.PI / 2) leftAngle = Math.max(leftAngle, openingHalfAngle);
    if (theta > Math.PI * 1.5) rightAngle = Math.min(rightAngle, Math.PI * 2 - openingHalfAngle);
    const angle = THREE.MathUtils.lerp(leftAngle, rightAngle, (u + 1) / 2);
