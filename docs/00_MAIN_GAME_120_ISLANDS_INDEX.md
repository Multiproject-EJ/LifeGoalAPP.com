...
**Objective:** M7-COMPLETE — Boss stop full production polish: ensure boss trial/rewards/state are production-ready for all users, telemetry wired, state persisted, QA coverage complete.
**Files to touch:** `src/features/gamification/level-worlds/components/IslandRunBoardPrototype.tsx`, related services
**Acceptance criteria:** Boss stop modal production-ready; boss trial resolve/reward feedback; boss reward persisted; island clear celebration; telemetry; `npm run build` passes.
**How to test:** Open Island Run; complete all 5 stops; open boss stop; resolve boss trial; verify reward feedback and island clear.
