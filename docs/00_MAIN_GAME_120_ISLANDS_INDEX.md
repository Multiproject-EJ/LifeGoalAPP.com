...
- [✅] M5: Hatchery + egg stages + dormant carryover (M5A egg scaffold shipped in prototype; M5-COMPLETE — egg stages 1–4 display correctly with stage names + pips; hatch delay random 24–72 h (surprise — no countdown); rollEggRewards canonical reward contract implemented; dormant carryover fixed — egg stays on origin island in per-island ledger, restored on revisit; hatchery stop modal fully production-grade; Island 1 forces egg set; handleSellEgg persists sold status in ledger)
...
**Objective:** M6-COMPLETE — Encounter tile (easy) + rewards production polish: ensure encounter tile events display correctly, rewards are granted reliably, and encounter resolution is fully production-ready for all users.
...
**Acceptance criteria:** Encounter tile triggers on landing; encounter modal shows challenge text; resolving grants rewards; encounter state persists; build passes.
**How to test:** Open Island Run; land on encounter tile (index 6); verify modal opens; resolve encounter; verify reward granted; verify encounter resolved state shown; travel and verify reset.
