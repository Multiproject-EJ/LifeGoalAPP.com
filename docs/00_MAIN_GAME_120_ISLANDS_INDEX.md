...
**Objective:** M16D: Collectible Progress Bar HUD pill component (era emoji + `islandShards / threshold` display, always visible in HUD)
...
**Acceptance criteria:** HUD pill renders era emoji (from `getShardEraEmoji`) + `islandShards / threshold` text always visible outside dev panel; pill updates reactively; `npm run build` passes.
**How to test:** load Island Run and observe pill in HUD header; earn shards and observe pill count update; verify emoji changes with tier index.
