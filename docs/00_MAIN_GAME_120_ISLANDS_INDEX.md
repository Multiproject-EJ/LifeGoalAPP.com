...
**Objective:** M9-COMPLETE — Home island + hatchery slots production polish: all hatchery actions production-ready, per-island egg slot enforcement, always-collect egg UX, telemetry wired.
**Files to touch:** `src/features/gamification/level-worlds/components/IslandRunBoardPrototype.tsx`, related hatchery services
**Acceptance criteria:** Hatchery slot UI production-ready; set/open/sell egg actions fully wired; per-island slot enforcement; `npm run build` passes.
**How to test:** Open Island Run; set egg in hatchery; wait for hatch; open egg; verify rewards; verify per-island slot enforcement on island travel.
