...
**Objective:** M8-COMPLETE — Market stop full production polish: ensure all market purchase flows, owned-state no-repurchase UX, telemetry, and QA coverage are production-ready for all users.
**Files to touch:** `src/features/gamification/level-worlds/components/IslandRunBoardPrototype.tsx`, related services
**Acceptance criteria:** Market stop modal production-ready; dice/heart bundle purchase flows complete; no-repurchase UX enforced; telemetry wired; `npm run build` passes.
**How to test:** Open Island Run; complete stops; open shop; purchase dice/heart bundle; verify no-repurchase block; verify telemetry events.
