...
- **Tier B (Caretaker Wheel Clue) — DONE.** The former Daily Clue Card is now a rare caretaker-led visual encounter. One clue tile exists only on every fifth island (5, 10, 15, ...), and the encounter may open once per island visit. `services/islandRunClueCardCurriculum.ts` still supplies chapter-aware questions; `IslandRunGamifiedJournalCard` presents them as two short visual picks instead of a long journal form. Cadence and topology are covered by `islandRunCardDrawCadence.test.ts` and `islandBoardTopology.test.ts`.
- **Within-island repetition — REMOVED BY DESIGN.** The prompt adapter still accepts `drawIndex` for compatibility, but the production cadence permits only one caretaker clue per milestone-island visit.
...
### Tier B — Caretaker Wheel Clue (rare milestone encounter)
The old ring-tile Daily Clue Card interrupted ordinary board play too frequently. Its replacement is intentionally scarce: one clue tile appears only on every fifth island and opens at most once per visit. The island caretaker introduces the moment through character art and a wheel visual; the player makes two quick visual picks and may add one optional line before saving. Chapter-aware questions still come from the per-chapter clue pool, so milestone encounters follow the six-chapter Compass Book arc without becoming a second long-form activity.
