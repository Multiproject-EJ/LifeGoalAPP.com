...
#### Backend / Data
- [~] **Full per-run state persistence** — stop flags already persist, and this slice now adds token index + hearts/coins/spin tokens to runtime-state storage; dice pool and broader integration verification still need follow-through and live-schema confirmation
...
| 5 | **M13A** | Full per-run state persistence — persist token index, stop flags, hearts, coins, spin tokens to Supabase on every mutation *(in progress: storage/runtime wiring landed; final integration verification remains)* | `islandRunRuntimeState.ts`, `islandRunGameStateStore.ts`, new migration |
