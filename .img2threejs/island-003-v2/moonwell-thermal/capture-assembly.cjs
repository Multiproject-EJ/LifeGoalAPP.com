...
(async () => {
  ...
  const cases = [['cold','event'],['half','event'],['hot','event'],['left','Left orbit'],['right','Right orbit']];
  let browser;
  try {
    ...
    for (const [name, view] of cases) {
      ...
      await page.goto(baseUrl + '&moonwellThaw=' + (name === 'hot' ? '1' : name === 'half' ? '0.5' : '0'), { waitUntil: 'domcontentloaded', timeout: 180000 });
