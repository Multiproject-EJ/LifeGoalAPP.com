...
function sourceSnapshot() {
  const files = git(['ls-files', '--', 'src/features/gamification/level-worlds/dev', 'src/features/gamification/level-worlds/services/islandRun3DWorldRouting.ts']).split('\n').filter(Boolean).concat(['src/features/gamification/level-worlds/dev/Island3FrozenWorldThreeModel.ts','src/features/gamification/level-worlds/dev/Island3FrostfireInspectionGeometry.ts','src/features/gamification/level-worlds/dev/Island3MoonwellThermalPresentation.ts']).sort();
...
(async () => {
  ...
  const cases = [['snowfeather-L2','hatchery'],['hearthguard-L2','habit']];
  let browser;
  try {
    ...
    for (const [name, view] of cases) {
      ...
      await page.goto(baseUrl.replace('level=3','level=2'), { waitUntil: 'domcontentloaded', timeout: 180000 });
