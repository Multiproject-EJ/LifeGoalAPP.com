...
exports.fetchPowerUpsCatalog = fetchPowerUpsCatalog;
exports.fetchUserPowerUps = fetchUserPowerUps;
exports.getActivePowerUps = getActivePowerUps;
exports.getActiveXPMultiplier = getActiveXPMultiplier;
exports.purchasePowerUp = purchasePowerUp;
exports.activatePowerUp = activatePowerUp;
exports.checkAndExpirePowerUps = checkAndExpirePowerUps;
exports.applyPermanentUpgrade = applyPermanentUpgrade;
const supabaseClient_1 = require("../lib/supabaseClient");
const gamificationPrefs_1 = require("./gamificationPrefs");
 ...
 */
async function fetchPowerUpsCatalog() {
 ...
 */
async function fetchUserPowerUps(userId) {
    if (!(0, supabaseClient_1.canUseSupabaseData)()) {
        try {
 ...
 */
async function getActivePowerUps(userId) {
 ...
 */
async function getActiveXPMultiplier(userId) {
 ...
 * Purchase a power-up
 */
async function purchasePowerUp(userId, powerUpId) {
    const { data: profile, error: profileError } = await (0, gamificationPrefs_1.fetchGamificationProfile)(userId);
    ...
    if (!(0, supabaseClient_1.canUseSupabaseData)()) {
        ...
        (0, gamificationPrefs_1.saveDemoProfile)({ ...profile, total_points: newBalance });
 ...
 */
async function applyInstantEffect(userId, powerUp, profile) {
    const supabase = (0, supabaseClient_1.getSupabaseClient)();
    switch (powerUp.effectType) {
        case 'instant_xp':
            await (0, gamification_1.awardXP)(userId, powerUp.effectValue, 'daily_login', undefined, `Power-up: ${powerUp.name}`);
            return `Awarded ${powerUp.effectValue} XP`;
        case 'extra_life':
            if (!(0, supabaseClient_1.canUseSupabaseData)()) {
                (0, gamificationPrefs_1.saveDemoProfile)({
                    ...profile,
        ...
        case 'mystery':
            ...
            if (randomReward.type === 'xp') {
                await (0, gamification_1.awardXP)(userId, randomReward.value, 'daily_login', undefined, 'Mystery Chest');
            }
            else if (randomReward.type === 'gold') {
                if (!(0, supabaseClient_1.canUseSupabaseData)()) {
                    (0, gamificationPrefs_1.saveDemoProfile)({ ...profile, total_points: profile.total_points + randomReward.value });
                }
                else {
                    await supabase
                        .from('gamification_profiles')
        ...
        case 'streak_freeze':
            if (!(0, supabaseClient_1.canUseSupabaseData)()) {
                ...
                const { data: currentProfile } = await supabase
                    ...
                    .eq('user_id', userId)
                    .single();
                const capacity = currentProfile?.freeze_bank_capacity || 3;
                await supabase
                    .from('gamification_profiles')
                    .update({
                    streak_freezes: Math.min((currentProfile?.streak_freezes || 0) + powerUp.effectValue, capacity),
                })
                    .eq('user_id', userId);
            }
 ...
 */
async function activatePowerUp(userPowerUpId, userId) {
    if (!(0, supabaseClient_1.canUseSupabaseData)()) {
        // Demo mode - mark as active in localStorage
        const stored = localStorage.getItem(`${DEMO_POWERUPS_KEY}_${userId}`);
        if (stored) {
            const powerups = JSON.parse(stored);
            const updated = powerups.map((p) => p.id === userPowerUpId
                ? {
                    ...p,
                    isActive: true,
 ...
 */
async function checkAndExpirePowerUps(userId) {
    const { data: userPowerUps, error } = await fetchUserPowerUps(userId);
    if (error || !userPowerUps) {
        return { data: null, error };
    }
 ...
 */
async function applyPermanentUpgrade(userId, effectType, value) {
    const supabase = (0, supabaseClient_1.getSupabaseClient)();
    if (!(0, supabaseClient_1.canUseSupabaseData)()) {
        // Demo mode
        const { data: profile, error: profileError } = await (0, gamificationPrefs_1.fetchGamificationProfile)(userId);
        ...
        (0, gamificationPrefs_1.saveDemoProfile)(updatedProfile);
        return { data: true, error: null };
    }
    // Supabase mode - use the database function
    try {
        const { error } = await supabase.rpc('apply_permanent_upgrade', {
            p_user_id: userId,
 ...
 */
async function deactivatePowerUp(userPowerUpId, userId) {
    if (!(0, supabaseClient_1.canUseSupabaseData)())
        return;
    const supabase = (0, supabaseClient_1.getSupabaseClient)();
    await supabase
...
function mapRowToUserPowerUp(row) {
    return {
        id: row.id,
        userId: row.user_id,
