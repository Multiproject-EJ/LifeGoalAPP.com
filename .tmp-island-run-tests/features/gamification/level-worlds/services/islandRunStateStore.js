...
exports.getIslandRunStateSnapshot = getIslandRunStateSnapshot;
exports.subscribeIslandRunState = subscribeIslandRunState;
exports.commitIslandRunState = commitIslandRunState;
exports.hydrateIslandRunState = hydrateIslandRunState;
exports.resetIslandRunStateSnapshot = resetIslandRunStateSnapshot;
exports.refreshIslandRunStateFromLocal = refreshIslandRunStateFromLocal;
exports.patchIslandRunStateSnapshot = patchIslandRunStateSnapshot;
exports.__resetIslandRunStateStoreForTests = __resetIslandRunStateStoreForTests;
exports.__getIslandRunStateSubscriberCountForTests = __getIslandRunStateSubscriberCountForTests;
const islandRunGameStateStore_1 = require("./islandRunGameStateStore");
const slotsByUser = new Map();
function getSlot(session) {
    const userId = session.user.id;
    let slot = slotsByUser.get(userId);
    if (!slot) {
        slot = {
            snapshot: (0, islandRunGameStateStore_1.readIslandRunGameStateRecord)(session),
            listeners: new Set(),
        };
 ...
 */
function getIslandRunStateSnapshot(session) {
 ...
 * listener itself.
 */
function subscribeIslandRunState(session, listener) {
 ...
 */
async function hydrateIslandRunState(options) {
 ...
 * is performed.
 */
function resetIslandRunStateSnapshot(session, record) {
 ...
 */
function refreshIslandRunStateFromLocal(session) {
    const slot = getSlot(session);
    const fresh = (0, islandRunGameStateStore_1.readIslandRunGameStateRecord)(session);
    publish(slot, fresh);
}
/**
 ...
 */
function patchIslandRunStateSnapshot(session, patch) {
    const slot = getSlot(session);
 ...
 */
function __getIslandRunStateSubscriberCountForTests(session) {
