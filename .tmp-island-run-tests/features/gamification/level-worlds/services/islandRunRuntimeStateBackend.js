...
exports.getIslandRunRuntimeStateBackend = getIslandRunRuntimeStateBackend;
const islandRunProfile_1 = require("./islandRunProfile");
const islandRunGameStateStore_1 = require("./islandRunGameStateStore");
const islandRunBonusTile_1 = require("./islandRunBonusTile");
const gameStateStorageBackend = {
    read(session) {
        return (0, islandRunGameStateStore_1.readIslandRunGameStateRecord)(session);
    },
    async hydrate({ session, client, forceRemote }) {
        return (0, islandRunGameStateStore_1.hydrateIslandRunGameStateRecord)({ session, client, forceRemote });
    },
    async hydrateWithSource({ session, client, forceRemote }) {
        const result = await (0, islandRunGameStateStore_1.hydrateIslandRunGameStateRecordWithSource)({ session, client, forceRemote });
        return { state: result.record, source: result.source };
    },
    async persistPatch({ session, client, patch }) {
        const hydratedBase = await (0, islandRunGameStateStore_1.hydrateIslandRunGameStateRecord)({ session, client });
        ...
        const freshLocal = (0, islandRunGameStateStore_1.readIslandRunGameStateRecord)(session);
        const current = freshLocal.runtimeVersion >= hydratedBase.runtimeVersion ? freshLocal : hydratedBase;
        const nextActiveStopIndex = typeof patch.activeStopIndex === 'number' && Number.isFinite(patch.activeStopIndex)
        ...
        const nextState = {
            ...
            dailyHeartsClaimedDayKey: typeof patch.dailyHeartsClaimedDayKey === 'string' || patch.dailyHeartsClaimedDayKey === null
                ? patch.dailyHeartsClaimedDayKey
                : current.dailyHeartsClaimedDayKey,
            onboardingDisplayNameLoopCompleted: typeof patch.onboardingDisplayNameLoopCompleted === 'boolean'
                ? patch.onboardingDisplayNameLoopCompleted
                : current.onboardingDisplayNameLoopCompleted,
            storyPrologueSeen: typeof patch.storyPrologueSeen === 'boolean'
                ? patch.storyPrologueSeen
                : current.storyPrologueSeen,
            audioEnabled: typeof patch.audioEnabled === 'boolean'
                ? patch.audioEnabled
                : current.audioEnabled,
            currentIslandNumber: typeof patch.currentIslandNumber === 'number' && Number.isFinite(patch.currentIslandNumber)
                ? Math.max(1, Math.floor(patch.currentIslandNumber))
                : current.currentIslandNumber,
            cycleIndex: typeof patch.cycleIndex === 'number' && Number.isFinite(patch.cycleIndex)
                ? Math.max(0, Math.floor(patch.cycleIndex))
                : current.cycleIndex,
            bossTrialResolvedIslandNumber: typeof patch.bossTrialResolvedIslandNumber === 'number' && Number.isFinite(patch.bossTrialResolvedIslandNumber)
            ...
            activeEggTier: patch.activeEggTier === null || patch.activeEggTier === 'common' || patch.activeEggTier === 'rare' || patch.activeEggTier === 'mythic'
                ? patch.activeEggTier
                : 'activeEggTier' in patch
                    ? null
                    : current.activeEggTier,
            activeEggSetAtMs: typeof patch.activeEggSetAtMs === 'number' && Number.isFinite(patch.activeEggSetAtMs)
                ? patch.activeEggSetAtMs
                : patch.activeEggSetAtMs === null
                    ? null
                    : current.activeEggSetAtMs,
            activeEggHatchDurationMs: typeof patch.activeEggHatchDurationMs === 'number' && Number.isFinite(patch.activeEggHatchDurationMs)
                ? patch.activeEggHatchDurationMs
                : patch.activeEggHatchDurationMs === null
                    ? null
                    : current.activeEggHatchDurationMs,
            activeEggIsDormant: typeof patch.activeEggIsDormant === 'boolean' ? patch.activeEggIsDormant : current.activeEggIsDormant,
            perIslandEggs: patch.perIslandEggs
                ? { ...current.perIslandEggs, ...patch.perIslandEggs }
                : current.perIslandEggs,
            islandStartedAtMs: typeof patch.islandStartedAtMs === 'number' && Number.isFinite(patch.islandStartedAtMs)
                ? patch.islandStartedAtMs
                : current.islandStartedAtMs,
            islandExpiresAtMs: typeof patch.islandExpiresAtMs === 'number' && Number.isFinite(patch.islandExpiresAtMs)
                ? patch.islandExpiresAtMs
                : current.islandExpiresAtMs,
            islandShards: typeof patch.islandShards === 'number' && Number.isFinite(patch.islandShards)
                ? Math.max(0, Math.floor(patch.islandShards))
                : current.islandShards,
            tokenIndex: typeof patch.tokenIndex === 'number' && Number.isFinite(patch.tokenIndex)
                ? Math.max(0, Math.floor(patch.tokenIndex))
                : current.tokenIndex,
            spinTokens: typeof patch.spinTokens === 'number' && Number.isFinite(patch.spinTokens)
                ? Math.max(0, Math.floor(patch.spinTokens))
                : current.spinTokens,
            dicePool: typeof patch.dicePool === 'number' && Number.isFinite(patch.dicePool)
                ? Math.max(0, Math.floor(patch.dicePool))
                : current.dicePool,
            shardTierIndex: typeof patch.shardTierIndex === 'number' && Number.isFinite(patch.shardTierIndex)
                ? Math.max(0, Math.floor(patch.shardTierIndex))
                : current.shardTierIndex,
            shardClaimCount: typeof patch.shardClaimCount === 'number' && Number.isFinite(patch.shardClaimCount)
                ? Math.max(0, Math.floor(patch.shardClaimCount))
                : current.shardClaimCount,
            shields: typeof patch.shields === 'number' && Number.isFinite(patch.shields)
                ? Math.max(0, Math.floor(patch.shields))
                : current.shields,
            shards: typeof patch.shards === 'number' && Number.isFinite(patch.shards)
                ? Math.max(0, Math.floor(patch.shards))
                : current.shards,
            diamonds: typeof patch.diamonds === 'number' && Number.isFinite(patch.diamonds)
                ? Math.max(0, Math.floor(patch.diamonds))
                : current.diamonds,
            creatureTreatInventory: patch.creatureTreatInventory !== null && typeof patch.creatureTreatInventory === 'object' && !Array.isArray(patch.creatureTreatInventory)
                ? {
            ...
            companionBonusLastVisitKey: typeof patch.companionBonusLastVisitKey === 'string' || patch.companionBonusLastVisitKey === null
                ? patch.companionBonusLastVisitKey
                : current.companionBonusLastVisitKey,
            completedStopsByIsland: patch.completedStopsByIsland !== null && typeof patch.completedStopsByIsland === 'object' && !Array.isArray(patch.completedStopsByIsland)
                ? {
                    ...current.completedStopsByIsland,
                    ...Object.fromEntries(Object.entries(patch.completedStopsByIsland).map(([islandKey, stops]) => [
                        islandKey,
                        Array.isArray(stops) ? stops.filter((stop) => typeof stop === 'string') : [],
                    ])),
                }
                : current.completedStopsByIsland,
            stopTicketsPaidByIsland: patch.stopTicketsPaidByIsland !== null && typeof patch.stopTicketsPaidByIsland === 'object' && !Array.isArray(patch.stopTicketsPaidByIsland)
                ? {
                    ...current.stopTicketsPaidByIsland,
                    ...Object.fromEntries(Object.entries(patch.stopTicketsPaidByIsland).map(([islandKey, ticketIndices]) => [
                        islandKey,
                        Array.isArray(ticketIndices)
                            ? Array.from(new Set(ticketIndices
                                .map((v) => Math.floor(v))
                                .filter((v) => Number.isFinite(v) && v > 0 && v < 5))).sort((a, b) => a - b)
                            : [],
                    ])),
                }
                : current.stopTicketsPaidByIsland,
            ...
            patch.bonusTileChargeByIsland !== null && typeof patch.bonusTileChargeByIsland === 'object' && !Array.isArray(patch.bonusTileChargeByIsland)
                ? {
                    ...
                    ...Object.fromEntries(Object.entries(patch.bonusTileChargeByIsland).map(([islandKey, inner]) => {
                        ...
                        for (const [idxKey, chargeRaw] of Object.entries(inner)) {
                            ...
                            if (!Number.isFinite(idx) || idx < 0)
                                continue;
                            const normalized = (0, islandRunBonusTile_1.clampBonusCharge)(chargeRaw);
                            if (normalized > 0)
                ...
                : current.bonusTileChargeByIsland,
            marketOwnedBundlesByIsland: patch.marketOwnedBundlesByIsland !== null && typeof patch.marketOwnedBundlesByIsland === 'object' && !Array.isArray(patch.marketOwnedBundlesByIsland)
                ? {
                    ...current.marketOwnedBundlesByIsland,
                    ...Object.fromEntries(Object.entries(patch.marketOwnedBundlesByIsland).map(([islandKey, bundles]) => [
                        islandKey,
                ...
                : current.marketOwnedBundlesByIsland,
            creatureCollection: Array.isArray(patch.creatureCollection)
                ? patch.creatureCollection
                : current.creatureCollection,
            activeCompanionId: typeof patch.activeCompanionId === 'string' || patch.activeCompanionId === null
                ? patch.activeCompanionId
                : current.activeCompanionId,
            perfectCompanionIds: Array.isArray(patch.perfectCompanionIds)
                ? patch.perfectCompanionIds.filter((id) => typeof id === 'string' && id.trim().length > 0)
                : current.perfectCompanionIds,
            perfectCompanionReasons: patch.perfectCompanionReasons !== null && typeof patch.perfectCompanionReasons === 'object' && !Array.isArray(patch.perfectCompanionReasons)
                ? Object.fromEntries(Object.entries(patch.perfectCompanionReasons).map(([creatureId, reason]) => [
                    creatureId,
            ...
            perfectCompanionComputedAtMs: typeof patch.perfectCompanionComputedAtMs === 'number' && Number.isFinite(patch.perfectCompanionComputedAtMs)
                ? patch.perfectCompanionComputedAtMs
                : patch.perfectCompanionComputedAtMs === null
                    ? null
                    : current.perfectCompanionComputedAtMs,
            perfectCompanionModelVersion: typeof patch.perfectCompanionModelVersion === 'string' || patch.perfectCompanionModelVersion === null
                ? patch.perfectCompanionModelVersion
                : current.perfectCompanionModelVersion,
            perfectCompanionComputedCycleIndex: typeof patch.perfectCompanionComputedCycleIndex === 'number' && Number.isFinite(patch.perfectCompanionComputedCycleIndex)
            ...
            activeStopType: patch.activeStopType === 'hatchery'
                ...
                : 'activeStopType' in patch
                    ? (0, islandRunGameStateStore_1.deriveIslandRunContractV2StopType)(nextActiveStopIndex)
                    : ('activeStopIndex' in patch ? (0, islandRunGameStateStore_1.deriveIslandRunContractV2StopType)(nextActiveStopIndex) : current.activeStopType),
            stopStatesByIndex: Array.isArray(patch.stopStatesByIndex)
            ...
            stopBuildStateByIndex: Array.isArray(patch.stopBuildStateByIndex)
                ? Array.from({ length: 5 }, (_, index) => {
                    ...
                    if (!entry)
                        return fallbackEntry;
                    return {
                        requiredEssence: typeof entry.requiredEssence === 'number' && Number.isFinite(entry.requiredEssence)
                            ? Math.max(0, Math.floor(entry.requiredEssence))
                            : fallbackEntry.requiredEssence,
                        spentEssence: typeof entry.spentEssence === 'number' && Number.isFinite(entry.spentEssence)
                            ? Math.max(0, Math.floor(entry.spentEssence))
                            : fallbackEntry.spentEssence,
                        buildLevel: typeof entry.buildLevel === 'number' && Number.isFinite(entry.buildLevel)
                            ? Math.max(0, Math.floor(entry.buildLevel))
                            : fallbackEntry.buildLevel,
                    };
                })
                : current.stopBuildStateByIndex,
            bossState: patch.bossState !== null && typeof patch.bossState === 'object' && !Array.isArray(patch.bossState)
                ? {
                ...
                : current.bossState,
            essence: typeof patch.essence === 'number' && Number.isFinite(patch.essence)
                ? Math.max(0, Math.floor(patch.essence))
                : current.essence,
            essenceLifetimeEarned: typeof patch.essenceLifetimeEarned === 'number' && Number.isFinite(patch.essenceLifetimeEarned)
                ? Math.max(0, Math.floor(patch.essenceLifetimeEarned))
                : current.essenceLifetimeEarned,
            essenceLifetimeSpent: typeof patch.essenceLifetimeSpent === 'number' && Number.isFinite(patch.essenceLifetimeSpent)
                ? Math.max(0, Math.floor(patch.essenceLifetimeSpent))
                : current.essenceLifetimeSpent,
            diceRegenState: patch.diceRegenState !== null
            ...
            rewardBarProgress: typeof patch.rewardBarProgress === 'number' && Number.isFinite(patch.rewardBarProgress)
                ? Math.max(0, Math.floor(patch.rewardBarProgress))
                : current.rewardBarProgress,
            rewardBarThreshold: typeof patch.rewardBarThreshold === 'number' && Number.isFinite(patch.rewardBarThreshold)
                ? Math.max(1, Math.floor(patch.rewardBarThreshold))
                : current.rewardBarThreshold,
            rewardBarClaimCountInEvent: typeof patch.rewardBarClaimCountInEvent === 'number' && Number.isFinite(patch.rewardBarClaimCountInEvent)
                ? Math.max(0, Math.floor(patch.rewardBarClaimCountInEvent))
                : current.rewardBarClaimCountInEvent,
            rewardBarEscalationTier: typeof patch.rewardBarEscalationTier === 'number' && Number.isFinite(patch.rewardBarEscalationTier)
                ? Math.max(0, Math.floor(patch.rewardBarEscalationTier))
                : current.rewardBarEscalationTier,
            rewardBarLastClaimAtMs: typeof patch.rewardBarLastClaimAtMs === 'number' && Number.isFinite(patch.rewardBarLastClaimAtMs)
                ? patch.rewardBarLastClaimAtMs
                : patch.rewardBarLastClaimAtMs === null
                    ? null
                    : current.rewardBarLastClaimAtMs,
            rewardBarBoundEventId: typeof patch.rewardBarBoundEventId === 'string' || patch.rewardBarBoundEventId === null
                ? patch.rewardBarBoundEventId
                : current.rewardBarBoundEventId,
            rewardBarLadderId: typeof patch.rewardBarLadderId === 'string'
                ? patch.rewardBarLadderId
                : patch.rewardBarLadderId === null
                    ? null
                    : current.rewardBarLadderId,
            activeTimedEvent: patch.activeTimedEvent !== null
            ...
            activeTimedEventProgress: patch.activeTimedEventProgress !== null
                && typeof patch.activeTimedEventProgress === 'object'
                && !Array.isArray(patch.activeTimedEventProgress)
                ? {
                    feedingActions: typeof patch.activeTimedEventProgress.feedingActions === 'number' && Number.isFinite(patch.activeTimedEventProgress.feedingActions)
                        ? Math.max(0, Math.floor(patch.activeTimedEventProgress.feedingActions))
                        : current.activeTimedEventProgress.feedingActions,
                    tokensEarned: typeof patch.activeTimedEventProgress.tokensEarned === 'number' && Number.isFinite(patch.activeTimedEventProgress.tokensEarned)
                        ? Math.max(0, Math.floor(patch.activeTimedEventProgress.tokensEarned))
                        : current.activeTimedEventProgress.tokensEarned,
                    milestonesClaimed: typeof patch.activeTimedEventProgress.milestonesClaimed === 'number' && Number.isFinite(patch.activeTimedEventProgress.milestonesClaimed)
            ...
            stickerProgress: patch.stickerProgress !== null && typeof patch.stickerProgress === 'object' && !Array.isArray(patch.stickerProgress)
                ? {
                    fragments: typeof patch.stickerProgress.fragments === 'number' && Number.isFinite(patch.stickerProgress.fragments)
            ...
            stickerInventory: patch.stickerInventory !== null && typeof patch.stickerInventory === 'object' && !Array.isArray(patch.stickerInventory)
                ? {
                    ...current.stickerInventory,
                    ...Object.fromEntries(Object.entries(patch.stickerInventory)
                ...
                : current.stickerInventory,
            lastEssenceDriftLost: typeof patch.lastEssenceDriftLost === 'number' && Number.isFinite(patch.lastEssenceDriftLost)
                ? Math.max(0, Math.floor(patch.lastEssenceDriftLost))
                : current.lastEssenceDriftLost,
            minigameTicketsByEvent: patch.minigameTicketsByEvent !== null
                ...
                ? (() => {
                    const merged = { ...current.minigameTicketsByEvent };
        ...
        const gameStatePersistResult = await (0, islandRunGameStateStore_1.writeIslandRunGameStateRecord)({
            session,
            client,
            record: nextState,
        });
        ...
        if (typeof patch.onboardingComplete === 'boolean') {
            return (0, islandRunProfile_1.persistIslandRunProfileMetadata)({
                session,
                client,
                metadataPatch: {
...
function getIslandRunRuntimeStateBackend() {
