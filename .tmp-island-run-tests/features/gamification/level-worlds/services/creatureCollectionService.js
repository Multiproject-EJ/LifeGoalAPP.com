...
exports.CREATURE_BOND_MILESTONE_LEVELS = exports.CREATURE_BOND_XP_PER_LEVEL = void 0;
exports.fetchCreatureCollection = fetchCreatureCollection;
exports.collectCreatureForUser = collectCreatureForUser;
exports.feedCreatureForUser = feedCreatureForUser;
exports.getUnclaimedBondMilestones = getUnclaimedBondMilestones;
exports.claimCreatureBondMilestoneForUser = claimCreatureBondMilestoneForUser;
exports.getCreatureManifestEntries = getCreatureManifestEntries;
exports.migrateLegacyEggLedgerToCollection = migrateLegacyEggLedgerToCollection;
exports.fetchActiveCompanionId = fetchActiveCompanionId;
exports.saveActiveCompanionId = saveActiveCompanionId;
exports.clearCreatureCollectionForUser = clearCreatureCollectionForUser;
const creatureCatalog_1 = require("./creatureCatalog");
exports.CREATURE_BOND_XP_PER_LEVEL = 3;
exports.CREATURE_BOND_MILESTONE_LEVELS = [3, 5, 8, 10];
function getStorageKey(userId) {
    return `island_run_creature_collection_${userId}`;
}
function getActiveCompanionStorageKey(userId) {
    return `island_run_active_companion_${userId}`;
}
function normalizeCollectionEntry(value) {
    if (typeof value.creatureId !== 'string' || !value.creatureId)
        return null;
    ...
    const bondXp = typeof value.bondXp === 'number' && Number.isFinite(value.bondXp)
        ...
        : 0;
    const derivedBondLevel = Math.floor(bondXp / exports.CREATURE_BOND_XP_PER_LEVEL) + 1;
...
function fetchCreatureCollection(userId) {
...
function writeCreatureCollection(userId, collection) {
    if (typeof window === 'undefined')
        return collection;
    try {
...
function collectCreatureForUser(options) {
...
function feedCreatureForUser(options) {
    ...
    const next = current.map((entry) => {
        ...
        return {
            ...entry,
            bondXp,
            bondLevel: Math.floor(bondXp / exports.CREATURE_BOND_XP_PER_LEVEL) + 1,
            lastFedAtMs: fedAtMs,
        };
    });
    return writeCreatureCollection(userId, next);
}
function getUnclaimedBondMilestones(entry) {
    const claimed = new Set(entry.claimedBondMilestones);
    return exports.CREATURE_BOND_MILESTONE_LEVELS.filter((milestone) => milestone <= entry.bondLevel && !claimed.has(milestone));
}
function claimCreatureBondMilestoneForUser(options) {
...
function getCreatureManifestEntries(userId) {
    return fetchCreatureCollection(userId)
        .map((entry) => {
        const creature = (0, creatureCatalog_1.getCreatureById)(entry.creatureId);
        if (!creature)
            return null;
...
function migrateLegacyEggLedgerToCollection(options) {
    ...
    Object.entries(perIslandEggs ?? {}).forEach(([islandKey, entry]) => {
        ...
        const creature = (0, creatureCatalog_1.selectCreatureForEgg)({ eggTier: entry.tier, seed: entry.setAtMs, islandNumber });
...
function fetchActiveCompanionId(userId) {
    if (typeof window === 'undefined')
        return null;
    try {
...
function saveActiveCompanionId(userId, creatureId) {
    if (typeof window === 'undefined')
        return;
    try {
 ...
 */
function clearCreatureCollectionForUser(userId) {
    if (typeof window === 'undefined')
        return;
    try {
