...
exports.drawEncounterChallengeForBucket = drawEncounterChallengeForBucket;
exports.drawEncounterChallenge = drawEncounterChallenge;
exports.rollEncounterReward = rollEncounterReward;
exports.formatEncounterRewardSummary = formatEncounterRewardSummary;
...
function resolveEncounterTypeFromSeed(seed, rarity) {
...
function drawEncounterChallengeForBucket(options) {
    const { islandNumber, tileIndex, timeBucket } = options;
    const seed = Math.abs((islandNumber * 31 + tileIndex * 7 + timeBucket) % 1000);
    const challengeType = resolveEncounterTypeFromSeed(seed, (0, islandBoardTileMap_1.getIslandRarity)(islandNumber));
...
function drawEncounterChallenge(islandNumber, tileIndex) {
...
function getEncounterTier(islandNumber) {
    if (islandNumber >= 60)
        return 3;
    if (islandNumber >= 30)
        return 2;
...
function rollEncounterReward(options) {
    ...
    const rarity = (0, islandBoardTileMap_1.getIslandRarity)(islandNumber);
...
function formatEncounterRewardSummary(reward) {
    const parts = [];
    if (reward.essence > 0)
    ...
    if (reward.walletShards)
        parts.push('+1 shard');
    if (reward.dice > 0)
        parts.push(`+${reward.dice} dice`);
    if (reward.spinTokens > 0)
        parts.push(`+${reward.spinTokens} spin`);
    return parts.join(' · ');
}
