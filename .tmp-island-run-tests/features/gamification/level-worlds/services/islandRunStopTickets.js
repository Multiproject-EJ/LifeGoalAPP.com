...
exports.STOP_COUNT = exports.STOP_TICKET_BASE_COSTS = void 0;
exports.getStopTicketCost = getStopTicketCost;
exports.getStopTicketsPaidForIsland = getStopTicketsPaidForIsland;
exports.isStopTicketPaid = isStopTicketPaid;
exports.payStopTicket = payStopTicket;
exports.sanitizeStopTicketsPaidByIsland = sanitizeStopTicketsPaidByIsland;
 ...
 */
exports.STOP_TICKET_BASE_COSTS = [
    0,
    30,
    70,
    130,
 ...
 */
function getStopTicketCost(options) {
    const idx = Math.max(0, Math.min(exports.STOP_COUNT - 1, Math.floor(options.stopIndex)));
    const base = exports.STOP_TICKET_BASE_COSTS[idx] ?? 0;
    if (base <= 0)
        return 0;
    const mult = (0, islandRunContractV2EssenceBuild_1.getIslandEssenceMultiplier)(Math.max(1, Math.floor(options.effectiveIslandNumber)));
 ...
 */
function getStopTicketsPaidForIsland(stopTicketsPaidByIsland, islandNumber) {
    if (!stopTicketsPaidByIsland)
        return [];
    ...
    for (const raw of entry) {
        ...
        if (!Number.isFinite(idx))
            continue;
        if (idx <= 0 || idx >= exports.STOP_COUNT)
            continue;
        if (seen.has(idx))
            continue;
        seen.add(idx);
...
function isStopTicketPaid(options) {
    if (options.stopIndex === 0)
        return true;
 ...
 */
function payStopTicket(options) {
    ...
    if (stopIndex < 0 || stopIndex >= exports.STOP_COUNT) {
    ...
    if (paid.includes(stopIndex))
        return { ok: false, reason: 'already_paid', cost };
    ...
    if (wallet < cost)
        return { ok: false, reason: 'insufficient_essence', cost };
    // Build the next map immutably.
    const nextMap = { ...(options.stopTicketsPaidByIsland ?? {}) };
...
function sanitizeStopTicketsPaidByIsland(stopTicketsPaidByIsland) {
    if (!stopTicketsPaidByIsland || typeof stopTicketsPaidByIsland !== 'object')
        return {};
    const out = {};
