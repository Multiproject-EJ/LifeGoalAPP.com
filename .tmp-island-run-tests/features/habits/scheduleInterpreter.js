...
exports.getISOWeekBounds = getISOWeekBounds;
exports.getTimesPerWeekProgress = getTimesPerWeekProgress;
exports.getEveryNDaysNextDue = getEveryNDaysNextDue;
exports.isHabitScheduledToday = isHabitScheduledToday;
exports.getScheduledCountForWindow = getScheduledCountForWindow;
/**
 * Type guard to check if a value is a valid schedule mode.
 */
function isValidScheduleMode(mode) {
...
function parseSchedule(schedule) {
    ...
    if (scheduleObj.mode === undefined && typeof scheduleObj.choice === 'string') {
        const choice = scheduleObj.choice;
        if (choice === 'every_day') {
            return { mode: 'daily' };
        }
        if (choice === 'specific_days') {
            return {
                mode: 'specific_days',
                days: Array.isArray(scheduleObj.days) ? scheduleObj.days : [],
            };
        }
    ...
    return {
        mode: scheduleObj.mode,
        days: Array.isArray(scheduleObj.days) ? scheduleObj.days : undefined,
 ...
 */
function countScheduledDaysInRange(startDate, endDate, days) {
    let count = 0;
 ...
 */
function getTimesPerWeekProgress(schedule, weekLogs) {
 ...
 */
function getEveryNDaysNextDue(schedule, habitCreatedAt, today = new Date()) {
 ...
 */
function isHabitScheduledToday(habit, today = new Date(), weekLogs = []) {
 ...
 */
function getScheduledCountForWindow(habit, windowDays, endDate = new Date()) {
    const schedule = parseSchedule(habit.schedule);
    if (!schedule || !schedule.mode) {
