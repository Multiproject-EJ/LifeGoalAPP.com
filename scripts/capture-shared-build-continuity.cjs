...
const sources=['Island5ThreePilot.tsx','IslandTemplateKitPage.tsx','IslandConstructionCommissioningFx.ts','Island3FrostmoonThreeWorld.ts'];
...
(async()=>{
 ...
 try {
  for(const [island,landmark,width,height,reduced] of [[3,'boss',390,844,false],[3,'wisdom',390,844,false],[1,'hatchery',390,844,false],[2,'habit',390,844,true],[15,'wisdom',390,844,false],[3,'boss',1280,800,false]].filter(item=>!process.argv.includes('--desktop-only') || item[2]===1280).slice(0,process.argv.includes('--phone-only')?0:process.argv.includes('--first')?1:6)){
   const start=snapshot(); const name=`${island}-${landmark}-${width}${reduced?'-reduced':''}`;
   const page=await browser.newPage({viewport:{width,height},deviceScaleFactor:1,reducedMotion:reduced?'reduce':'no-preference'});
   ...
   await page.goto(`http://127.0.0.1:53303/dev/island-template-kit?island=${island}&mode=3d&level=2&construction=1&working=1&constructionProgress=1&landmark=${landmark}&cameraAuthoring=1&reduced=${reduced?1:0}`);
   ...
   await click('Construction closed');await page.waitForTimeout(100);await click('Build Wisdom immediately');await page.waitForTimeout(600);states.reopen=await read();
   ...
   records.push({name,states,samples,finaleWidthRatio:ratio,sourceHashes:start});console.log('PASS',name,ratio);await page.close();
  }
