...
const DEFAULT_OUT = path.resolve(ROOT, 'work/island-visual-library/island-014-honeycomb-kingdom/evidence/palace-integrated-honeycomb-cathedral-v001-r01');
const OUT = path.resolve(ROOT, process.argv[2] ?? path.relative(ROOT, DEFAULT_OUT));
const IS_FREEZE = OUT === DEFAULT_OUT;
const VALIDATION = path.resolve(ROOT, '.gauntlet/island-014-honeycomb-kingdom-parts/parts/palace-integrated-honeycomb-cathedral/build-validation.v1.json');
...
const CONTRACT = path.resolve(ROOT, '.gauntlet/island-014-honeycomb-kingdom-parts/parts/palace-integrated-honeycomb-cathedral/build-contract.v1.json');
...
const EXPECTED = {
  ...
  contract: '31a6f2a63b4da8fac834498b420a34745e72726225eb54e46aa094053e9b203f',
};
...
function cameraFor(degrees, width, height, mode = 'orbit') {
  ...
  const target = new THREE.Vector3(0, close ? 2.35 : 2.48, close ? 0.34 : 0.02);
  const radius = close ? 6.45 : phone ? 15.2 : 9.2;
  const angle = THREE.MathUtils.degToRad(degrees);
  const camera = new THREE.PerspectiveCamera(close ? 31 : phone ? 42 : 35, width / height, 0.1, 100);
  camera.position.set(
    target.x + Math.sin(angle) * radius,
    close ? 4.25 : phone ? 9.15 : 5.65,
...
function materialRgb(material, light, materialsOn) {
  ...
  const color = source.clone().multiplyScalar(0.32 + light * 0.82);
  if (material?.emissive instanceof THREE.Color && material.emissiveIntensity > 0) {
    color.add(material.emissive.clone().multiplyScalar(Math.min(0.38, material.emissiveIntensity * 0.26)));
  }
...
function renderSvg(root, degrees, options) {
  ...
  root.traverse((object) => {
    ...
    for (let triangleIndex = 0; triangleIndex < triangleCount; triangleIndex += 1) {
      ...
      cameraDirection.copy(camera.position).sub(centroid);
      if (normal.dot(cameraDirection) <= 0.001) continue;
      ...
      triangles.push({
        depth,
        opacity: materialsOn && object.material?.transparent ? Math.max(0.78, object.material.opacity ?? 1) : 1,
  ...
  const polygons = triangles.map((triangle) => (
    `<polygon points="${triangle.points}" fill="${triangle.fill}" fill-opacity="${triangle.opacity.toFixed(3)}" stroke="${materialsOn ? '#562206' : '#70511d'}" stroke-opacity="${materialsOn ? '0.20' : '0.40'}" stroke-width="0.25" stroke-linejoin="round"/>`
  )).join('\n');
  const floorY = mode === 'phone' ? height * 0.80 : height * 0.87;
  const floorRx = mode === 'phone' ? width * 0.41 : width * 0.33;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  ${polygons}
  <text x="18" y="${height - 18}" fill="#dbe8f4" opacity="0.70" font-family="system-ui,sans-serif" font-size="${mode === 'phone' ? 11 : 14}">${xml(`actual integrated runtime mesh · ${String(degrees).padStart(3, '0')}° · materials ${materialsOn ? 'on' : 'off'}`)}</text>
</svg>`;
}
...
function meshMetrics(root) {
  ...
  root.traverse((object) => {
    ...
    if (/PROJECTION|FACADE_CARD|TRUE_360|CROSS_PLAN|DOME_SHELL|ROSE_BACKPLANE/i.test(object.name ?? '')) forbiddenLiveNames.push(object.name);
  });
  return { meshCount, triangleCount, openPlaneCount, forbiddenLiveNames };
}

async function rasterizeSvg(svgPath, pngPath, rasterDir) {
  await execFileAsync('/usr/bin/qlmanage', ['-t', '-s', '900', '-o', rasterDir, svgPath], { maxBuffer: 1024 * 1024 * 4 });
  const generated = path.join(rasterDir, `${path.basename(svgPath)}.png`);
  await fs.copyFile(generated, pngPath);
}
...
try {
  ...
  for (const degrees of ORBITS) {
    ...
    views.push({ id: `orbit-${String(degrees).padStart(3, '0')}-materials-on`, degrees, mode: 'materials-on', file });
  }
  for (const degrees of [0, 45, 315]) {
    ...
    views.push({ id: `orbit-${String(degrees).padStart(3, '0')}-materials-off`, degrees, mode: 'materials-off', file });
  }
  ...
  for (const item of views) {
    ...
    item.rasterFile = item.file.replace(/\.svg$/u, '.png');
    await rasterizeSvg(path.join(OUT, item.file), path.join(OUT, item.rasterFile), rasterDir);
    item.rasterSha256 = await sha(path.join(OUT, item.rasterFile));
  }
  ...
  const cathedral = palace.userData.integratedHoneycombCathedral;
  const checks = {
    ...
    correctConstructionFamily: cathedral?.constructionFamily === 'threejs-integrated-extruded-honeycomb-exoskeleton-cathedral',
    runtimeProjectionCountZero: cathedral?.runtimeProjectionCount === 0,
    facadeCardCountZero: cathedral?.facadeCardCount === 0,
    smoothDomeCountZero: cathedral?.smoothDomeCount === 0,
    ...
    totalHeightRatioInContract: cathedral?.totalHeightRatio >= 1.10 && cathedral?.totalHeightRatio <= 1.18,
    royalDoorWidthInContract: cathedral?.outerDoorWidth >= 1.05 && cathedral?.outerDoorWidth <= 1.12,
    royalRevealDepthAtLeast018: cathedral?.royalRevealDepth >= 0.18,
    purpleDoorWidthInContract: cathedral?.purpleDoorWidth >= 0.45 && cathedral?.purpleDoorWidth <= 0.50,
    purpleDoorRatioInContract: cathedral?.purpleDoorWidthHeightRatio >= 0.60 && cathedral?.purpleDoorWidthHeightRatio <= 0.70,
    embeddedRoseWidthInContract: cathedral?.embeddedRoseWidth >= 0.76 && cathedral?.embeddedRoseWidth <= 0.84,
    doorRoseRatioInContract: cathedral?.doorRoseRatio >= 1.15 && cathedral?.doorRoseRatio <= 1.35,
    fourFrontBays: cathedral?.frontDominantBayCount === 4,
    fourQuadrantSatellites: cathedral?.quadrantSatelliteCount === 4,
    authoredHoneyDripAssembly: cathedral?.honeyDripConstruction === 'upper-fillet+tapering-stream+terminal-drop',
    allElevationsAuthored: ['front', 'left', 'right', 'rear'].every((view) => cathedral?.authoredElevations?.includes(view)),
    runtimeDrawCallsAtMost48: metrics.meshCount <= 48,
    trianglesAtMost70000: metrics.triangleCount <= 70000,
    allFirstGateEvidencePresent: views.length === 13,
  ...
  const manifest = {
    ...
    representation: 'deterministic CPU-projected evidence of the actual batched L3 high runtime mesh; closed extruded geometry with zero runtime image projection',
    ...
    cameras: {
      orbitDegrees: ORBITS,
      orbit: { width: 900, height: 900, fov: 35, radius: 9.2, target: [0, 2.48, 0.02] },
      phone: { simulatedViewport: [390, 844], reviewCanvas: [900, 900], fov: 42, radius: 15.2, target: [0, 2.48, 0.02] },
    },
    cathedral,
    metrics,
    views,
  ...
  for (const view of views) {
    ...
    verifiedViews.push({
      ...
      svgSha256,
      rasterSha256,
      svgMatches: svgSha256 === view.sha256,
      rasterMatches: rasterSha256 === view.rasterSha256,
    });
  }
  const verification = {
  ...
  const validation = {
    ...
    checks,
    verification,
    passed: Object.values(checks).every(Boolean) && verification.runtimeMatches && verification.evidenceMismatchCount === 0 && verification.evidenceFileCount === 26,
  ...
  console.log(`Rendered and rasterized ${views.length} integrated-cathedral evidence views to ${OUT}`);
