...
async function main() {
  ...
  const supabaseUrl = process.env.VITE_SUPABASE_URL?.trim() || defaultCredentials.url;
  const supabaseAnonKey = process.env.VITE_SUPABASE_ANON_KEY?.trim() || defaultCredentials.anonKey;
  const supabaseRedirect = process.env.VITE_SUPABASE_REDIRECT_URL?.trim();
