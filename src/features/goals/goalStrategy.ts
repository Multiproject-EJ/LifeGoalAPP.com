...
export interface GoalStrategyMeta {
  label: string;
  icon: string;
  tagline: string;
  description: string;
...
export const GOAL_STRATEGY_META: Record<GoalStrategyType, GoalStrategyMeta> = {
  standard: {
    icon: '🎯',
    label: 'Standard',
...
export const GOAL_STRATEGY_ORDER: GoalStrategyType[] = [
  'standard',
  'micro',
  'anti_goal',
  'process',
...
export const GOAL_STRATEGY_OPTIONS = GOAL_STRATEGY_ORDER.map((value) => ({
  value,
  label: GOAL_STRATEGY_META[value].label,
  icon: GOAL_STRATEGY_META[value].icon,
  tagline: GOAL_STRATEGY_META[value].tagline,
}));

export const DEFAULT_GOAL_STRATEGY: GoalStrategyType = 'standard';

export function normalizeGoalStrategy(value: string | null | undefined): GoalStrategyType {
  if (!value) {
    return DEFAULT_GOAL_STRATEGY;
  }

  if (GOAL_STRATEGY_ORDER.includes(value as GoalStrategyType)) {
    return value as GoalStrategyType;
  }

  return DEFAULT_GOAL_STRATEGY;
}

export function getStrategyXpMultiplier(strategyType: string | null | undefined): number {
  return GOAL_STRATEGY_META[normalizeGoalStrategy(strategyType)].xpMultiplier;
}
