...
type HaircutWidgetProps = {
  userId: string;
};

/**
 * Self-contained "My Haircut" tracker shown on the Body & Style visionary tab.
 * Owns its own state and persists to localStorage (per user) via
 * haircutPreferences.
 */
export function HaircutWidget({ userId }: HaircutWidgetProps) {
  ...
  }, [userId, haircutIntervalDays, lastHaircutDate, selectedHaircutStyle, bestHairLength, needsHaircut]);
      ...
      >
        <div>
          ...
          <p className="vision-board__haircut-subtitle">
            Interval:{' '}
            {HAIRCUT_INTERVAL_OPTIONS.find((option) => option.value === haircutIntervalDays)?.label ??
              'Custom'}{' '}
        ...
        <div className="vision-board__haircut-details">
          ...
          <div className="vision-board__haircut-section">
            ...
            <div className="vision-board__haircut-inputs">
              ...
              <label>
                Best length
                <select value={bestHairLength} onChange={(event) => setBestHairLength(event.target.value)}>
            ...
            <p
              className={`vision-board__haircut-status ${
                needsHaircut ? 'vision-board__haircut-status--alert' : ''
              }`}
            >
              {needsHaircut
                ? 'Time for a trim — consider booking a cut.'
                : 'On track with your ideal length.'}
            </p>
          </div>
        </div>
      )}
