...
export interface IslandMissionObjectivesPresentation {
...
export function resolveLandmarkProgress(options: {
  islandNumber: number;
...
export function resolveIslandMissionObjectives(options: {
  islandNumber: number;
  state: MissionTrackerState;
  landmarkCount?: number;
    ...
    case 'arena_guardian': {
      const guardianDefeated = state.currentIslandNumber === islandNumber
        && (
          ...
          || state.completedStopsByIsland?.[String(islandNumber)]?.includes('boss') === true
        );
      objectives = [
