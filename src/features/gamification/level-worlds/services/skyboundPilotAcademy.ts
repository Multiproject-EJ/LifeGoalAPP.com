...
export const SKYBOUND_LESSONS: readonly SkyboundLesson[] = [
  {id:'cadet_launch',rankId:'cadet',index:0,globalIndex:0,name:'Launch Drill',shortName:'Launch',briefing:'Build a clean sling launch and follow the first crest line.',instructorTip:'Pull low for power, then release at 35–45°.',levelId:'meadow',goalDistance:190,standards:standards('cadet-launch',160,0,3),exam:false},
  {id:'cadet_gates',rankId:'cadet',index:1,globalIndex:1,name:'Precision Gates',shortName:'Gates',briefing:'Hold a smooth line through the cyan training gates.',instructorTip:'Small steering inputs preserve speed.',levelId:'meadow',goalDistance:270,standards:[{id:'cadet-gates-distance',kind:'distance',label:'Reach 220m',target:220},{id:'cadet-gates-rings',kind:'rings',label:'Clear 1 ring',target:1},{id:'cadet-gates-salvage',kind:'salvage',label:'Collect 4 crests',target:4}],exam:false},
...
export function settleSkyboundAcademyLesson(progress:SkyboundAcademyProgress,evaluation:SkyboundLessonEvaluation):SkyboundAcademyProgress { const lesson=getSkyboundLesson(evaluation.lessonId); const wasComplete=progress.completedLessonIds.includes(lesson.id); const completedLessonIds=evaluation.passed?unique([...progress.completedLessonIds,lesson.id]):progress.completedLessonIds; const aceLessonIds=evaluation.ace?unique([...progress.aceLessonIds,lesson.id]):progress.aceLessonIds; let promotedRankIds=progress.promotedRankIds; let medalRankIds=progress.medalRankIds; let tickets=progress.tickets; if(evaluation.passed&&lesson.exam&&!wasComplete){medalRankIds=unique([...medalRankIds,lesson.rankId]);const nextRank=SKYBOUND_AIRCRAFT_RANKS[rankIndex(lesson.rankId)+1];if(nextRank)promotedRankIds=unique([...promotedRankIds,nextRank.id]);tickets+=getSkyboundRank(lesson.rankId).rewardTickets;} const certificateAwarded=progress.certificateAwarded||(evaluation.passed&&lesson.id==='ace_exam'); return{...progress,tickets,academyXp:progress.academyXp+evaluation.academyXpEarned,completedLessonIds,aceLessonIds,promotedRankIds,medalRankIds,certificateAwarded}; }
