...
export type SkyboundLessonStandardKind = 'distance' | 'salvage' | 'rings' | 'hazards' | 'flow' | 'landing';
...
export const SKYBOUND_LESSONS: readonly SkyboundLesson[] = [
  {id:'cadet_launch',rankId:'cadet',index:0,globalIndex:0,name:'First Hop',shortName:'Hop',briefing:'Leave the grass, hold a shallow climb, and see how far the fuselage can travel.',instructorTip:'Pull low, release near 30–35°, and keep the field in sight.',levelId:'meadow',goalDistance:360,standards:standards('cadet-launch',120,0,3,0),exam:false},
...
export function getSkyboundLessonStandardResults(lessonId:SkyboundLessonId,flight:SkyboundFlightState):readonly SkyboundLessonStandardResult[] {
  const lesson=getSkyboundLesson(lessonId);
  return lesson.standards.map((standard)=>{
    const value=standard.kind==='distance'
      ? Math.round(Math.min(flight.x,flight.goalDistance))
      : standard.kind==='salvage'
        ? flight.salvageCollected
        : standard.kind==='rings'
          ? flight.ringsCleared
          : standard.kind==='flow'
            ? Math.floor(flight.smoothFlightMs/1000)
            : standard.kind==='landing'
              ? Number(flight.status==='landed')
              : flight.hazardHits+Math.min(3,flight.terrainImpacts);
    return{...standard,value,met:standard.kind==='hazards'?value<=standard.target:value>=standard.target};
  });
}
export function evaluateSkyboundLesson(lessonId:SkyboundLessonId,flight:SkyboundFlightState):SkyboundLessonEvaluation { const lesson=getSkyboundLesson(lessonId); const standardResults=getSkyboundLessonStandardResults(lessonId,flight); const standardsMet=standardResults.filter((standard)=>standard.met).length; const coreSkillMet=standardResults[1]?.met===true; const landingDistanceMet=standardResults[1]?.kind!=='landing'||standardResults[0]?.met===true; const passed=standardsMet>=2&&coreSkillMet&&landingDistanceMet; const ace=standardsMet===3; return{lessonId,standardResults,standardsMet,passed,ace,academyXpEarned:25+standardsMet*15+(passed?25:0)+(ace?25:0)+(lesson.exam&&passed?50:0)}; }
