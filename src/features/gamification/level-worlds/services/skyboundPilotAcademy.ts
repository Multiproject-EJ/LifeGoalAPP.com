...
export const SKYBOUND_LESSONS: readonly SkyboundLesson[] = [
  ...
  {id:'cadet_weather',rankId:'cadet',index:2,globalIndex:2,name:'Field Crosswind',shortName:'Wind',briefing:'Use Stabilizer against the practice-field fans without touching the grass.',instructorTip:'Stabilize briefly; holding it costs speed.',levelId:'meadow',goalDistance:330,standards:standards('cadet-weather',250,1,3),exam:false},
  ...
  {id:'trainee_landing',rankId:'trainee',index:2,globalIndex:6,name:'Landing Pattern',shortName:'Pattern',briefing:'Fly the coastal circuit and return the trainer to the field safely.',instructorTip:'Stabilizer settles the aircraft before the last gate.',levelId:'coast',goalDistance:580,standards:standards('trainee-pattern',520,2,5,0),exam:false},
...
const standardValue=(kind:SkyboundLessonStandardKind,flight:SkyboundFlightState)=>kind==='distance'?Math.round(Math.min(flight.x,flight.goalDistance)):kind==='salvage'?flight.salvageCollected:kind==='rings'?flight.ringsCleared:flight.hazardHits+Math.min(3,flight.terrainImpacts);
