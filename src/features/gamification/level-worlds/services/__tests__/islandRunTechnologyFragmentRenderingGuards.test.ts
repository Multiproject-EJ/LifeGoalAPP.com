...
export const islandRunTechnologyFragmentRenderingGuardTests: TestCase[] = [
  {
    ...
    run: () => {
      ...
      assertIncludes(boardTile, 'technologyFragment ? `Tile ${index + 1}. ${technologyFragment.ariaLabel}`', 'tile retains an accessible fragment fallback');
      assertIncludes(boardTileGrid, '? `Tile ${index + 1}. ${technologyFragment.ariaLabel}. Tap for details.`', 'inspectable tile label exposes full fragment availability');
