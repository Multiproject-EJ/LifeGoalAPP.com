...
export const islandRunConcordHubEntryTests: TestCase[] = [
  ...
  },
  {
    ...
    run: () => {
      ...
      assertEqual(state.label, 'Story', 'pre-build label exposes the collection goal');
  ...
  },
  {
    ...
    run: () => {
      ...
      assertEqual(fullGridWithoutUnlock.label, 'Story', 'full grid exposes complete restoration progress');
      assertEqual(fullGridWithoutUnlock.primaryAction, 'open-story', 'full grid alone does not open the active hub');

      const active = resolveIslandRunConcordHubEntryState({
        ...baseRecord(),
        technologyUnlocksById: { 'the-concord': { builtAtMs: 123, active: true } },
      });
      assertEqual(active.label, 'Concord & Story', 'active unlock changes label');
