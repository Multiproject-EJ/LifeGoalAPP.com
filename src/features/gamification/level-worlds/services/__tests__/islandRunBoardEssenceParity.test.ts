...
export const islandRunBoardEssenceParityTests: TestCase[] = [
...

  {
    ...
    run: async () => {
      ...
      assert(source.includes('Your First Signals Have Begun'), 'First progress recap should introduce personal signals before the soft save prompt.');
  ...
  {
    name: 'retired story rewards and sanctuary duplicate handlers remain removed',
    run: async () => {
      const source = await readBoardSource();
      assert(
      ...
      assert(
        !source.includes('onRewardClaim='),
        'Onboarding and island StoryReader episodes must remain presentation-only.',
      );
      assert(
