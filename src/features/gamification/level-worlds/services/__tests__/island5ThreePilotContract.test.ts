...
export const island5ThreePilotContractTests: TestCase[] = [
  ...
  },
  {
    ...
    run: async () => {
      ...
      assert(celestialSource.includes("'./Island2CelestialV2Landscape'") && celestialSource.includes('addCelestialV2Waterways(mainShelf,') && celestialSource.includes('addCelestialV2Waterways(shelf, plot,'), 'Island 002 must assemble connected waterways on the mainland and each docking district');
      assert(celestialSource.includes("'./Island2CelestialV2Terrain'") && celestialSource.includes('const shelf = createCelestialSkyRoot('), 'Island 002 must build its floating shelves through the V2 terrain factory');
      ...
      assert(/const showAssemblyCutaway = isAssemblyCraterFirstLight && preset === 'boss'\s*&& firstLightAssemblyCraterPresentationRef\.current\.completed;/.test(pilotSource), 'the sliced-open cutaway is reserved for the finished Assembly and its final blast, never early crater visits');
