...
export const islandRun3DWorldRoutingTests: TestCase[] = [
  ...
  },
  {
    ...
    run: () => {
      assertEqual(ISLAND_RUN_3D_WORLD_ROUTES.length, 11, 'eleven authored world packs are currently routed');
      assertEqual(new Set(ISLAND_RUN_3D_WORLD_ROUTES.map((route) => route.runtimeIslandNumber)).size, 11, 'runtime islands are unique');
      assertEqual(new Set(ISLAND_RUN_3D_WORLD_ROUTES.map((route) => route.worldSourceNumber)).size, 11, 'visual source packs are unique');
    },
  },
];
