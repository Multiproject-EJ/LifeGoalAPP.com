...
export function applyIslandRunTechnologyBuild(options: {
  ...
  source: 'island-5-tech-grid-completed' | 'island-1-tech-grid-completed' | 'compatibility-hydration';
  nowMs?: number;
...
export function applyDevClearCurrentIslandForTravel(
  ...
  if (normalizedIslandNumber === 5 && !workingRecord.technologyUnlocksById?.['the-concord']?.active) {
