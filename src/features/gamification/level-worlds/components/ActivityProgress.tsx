...
export function ActivityProgress({ completed, total, unit = 'answer', buildLevel, saved = false }: {
  completed: number; total: number; unit?: string; buildLevel?: number; saved?: boolean;
}) {
  ...
  return <section className="activity-progress" aria-label="Activity and construction progress">
    <div className="activity-progress__label">
      <strong>{remaining ? `${remaining} ${unit}${remaining === 1 ? '' : 's'} to go` : saved ? 'Activity saved' : 'Ready to submit'}</strong>
