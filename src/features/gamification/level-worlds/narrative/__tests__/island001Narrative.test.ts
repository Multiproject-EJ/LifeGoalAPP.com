...
export const island001NarrativeTests: TestCase[] = [
  ...
  { name: 'travel-ready beat has display CTA text but no travel action', run: () => { const travelReady = beat('I001-B30'); assertEqual(travelReady.displayCtaText, 'Follow the restored route', 'Expected display CTA'); assert(!Object.prototype.hasOwnProperty.call(travelReady, 'travel'), 'Travel-ready beat must not include travel action'); assert(!Object.prototype.hasOwnProperty.call(travelReady, 'actionId'), 'Travel-ready beat must not include actionId'); } },
  ...
  { name: 'approved Island 1 characters, shipboard PA, Central Command, and first-contact Caretaker are included', run: () => assertEqual(island001NarrativeDefinition.characters.map((character) => character.id).join(','), 'miri,sava,poko,ivo,noctyra,pa,central-command,luma-caretaker', 'Expected approved character IDs') },
  { name: 'Concord completion creates a one-time Caretaker first contact', run: () => { const firstContact = beat('I001-B32'); assertEqual(JSON.stringify(firstContact.trigger), JSON.stringify({ kind: 'technology_fragment_collected', islandNumber: 1, technologyId: 'the-concord', collectedCount: 9 }), 'Expected ninth-fragment trigger'); assertEqual(firstContact.speakerId, 'luma-caretaker', 'Expected Caretaker speaker'); assertEqual(firstContact.repeatPolicy, 'once', 'First contact must not replay after acknowledgement'); } },
  { name: 'Hatchery L1 explains reciprocal diplomatic supplies through the PA', run: () => { assertEqual(beat('I001-B24').speakerId, 'pa', 'Expected PA speaker'); assert(String(beat('I001-B24').text).includes('Reconstruction Accord'), 'Expected Accord explanation'); } },
  ...
  { name: 'wisdom reveal carries the non-judgmental restoration framing', run: () => { const b = beat('I001-B20'); assertEqual(b.secondaryText, 'Aim to free her, not to fight her.', 'Expected reveal framing'); assertEqual(b.priority, 'major', 'Reveal is a major beat'); } },
