...
export const island001NarrativeTests: TestCase[] = [
  ...
  { name: 'approved Island 1 characters, shipboard PA, and Central Command are included', run: () => assertEqual(island001NarrativeDefinition.characters.map((character) => character.id).join(','), 'miri,sava,poko,ivo,noctyra,pa,central-command', 'Expected approved character IDs') },
