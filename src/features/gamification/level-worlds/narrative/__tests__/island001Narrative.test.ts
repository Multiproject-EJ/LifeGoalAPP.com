...
export const island001NarrativeTests: TestCase[] = [
  ...
  { name: 'approved Island 1 characters and shipboard PA are included', run: () => assertEqual(island001NarrativeDefinition.characters.map((character) => character.id).join(','), 'miri,sava,poko,ivo,noctyra,pa', 'Expected approved character IDs') },
