...
export const islandNarrativeValidationTests: TestCase[] = [
  ...
  { name: 'story reader beat without episode path fails validation', run: () => expectInvalid((definition) => { const beat = definition.beats.find((entry: any) => entry.id === 'I001-B02'); delete beat.episodePath; }, 'requires episodePath') },
  { name: 'dialogue beat without text fails validation', run: () => expectInvalid((definition) => { const beat = definition.beats.find((entry: any) => entry.id === 'I001-B03'); delete beat.text; }, 'requires text') },
  { name: 'Expedition Phone beat without mission heading fails validation', run: () => expectInvalid((definition) => { const beat = definition.beats.find((entry: any) => entry.id === 'I001-B31'); delete beat.headline; }, 'requires headline') },
