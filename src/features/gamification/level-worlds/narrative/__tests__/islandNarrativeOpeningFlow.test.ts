...
import { isDiplomaticRewardChannelVisible } from '../islandDiplomaticPresentation';
...
export const islandNarrativeOpeningFlowTests: TestCase[] = [
  ...
  { name: 'opening briefing enters gameplay without auto-playing the arrival Webtoon', run: () => { assertIncludes(hookSource, 'isOpeningBriefingComplete', 'Opening beats should wait for the replacement briefing'); assert(!hookSource.includes("enqueueBeat('I001-B02')"), 'Onboarding must not auto-queue a StoryReader slideshow'); assertIncludes(hookSource, 'isNarrativeSurfaceBlocked', 'Opening should gate on modal collisions'); } },
  { name: 'Hatchery intro uses activeStopId transition only', run: () => { assertIncludes(hookSource, "activeStopId !== 'hatchery'", 'Expected active hatchery stop gate'); ['tileIndex', 'TOKEN_START_TILE_INDEX', 'STOP_TILE_INDICES_36'].forEach((needle) => assert(!hookSource.includes(needle), `Hatchery intro must not use ${needle}`)); } },
  ...
  { name: 'diplomatic reward channel appears only after the first follow-up order is accepted', run: () => { assert(!isDiplomaticRewardChannelVisible({ currentIslandNumber: 1, cycleIndex: 0, firstSessionTutorialState: 'awaiting_first_roll' }), 'Fresh Island 1 should hide reward channel'); assert(!isDiplomaticRewardChannelVisible({ currentIslandNumber: 1, cycleIndex: 0, firstSessionTutorialState: 'first_fragment_collected' }), 'Incoming DIPLOMATIC EFFORT order should keep the channel hidden'); assert(isDiplomaticRewardChannelVisible({ currentIslandNumber: 1, cycleIndex: 0, firstSessionTutorialState: 'first_roll_consumed' }), 'Acknowledging the follow-up order should reveal reward channel'); assert(isDiplomaticRewardChannelVisible({ currentIslandNumber: 2, cycleIndex: 0, firstSessionTutorialState: 'not_started' }), 'Later islands should show reward channel'); assert(isDiplomaticRewardChannelVisible({ currentIslandNumber: 1, cycleIndex: 1, firstSessionTutorialState: 'not_started' }), 'Repeat cycle should show reward channel'); } },
  { name: 'diplomatic activation gates and animates the core reward channel', run: () => { assertIncludes(boardSource, 'const diplomaticRewardChannelVisible = isDiplomaticRewardChannelVisible({', 'Board should derive the canonical order gate'); assertIncludes(boardSource, '{diplomaticRewardChannelVisible ? (', 'Core reward channel should remain absent before mission acceptance'); assertIncludes(boardSource, 'island-run-board__rewardbar-cluster--mission-reveal', 'Reward bar should receive the one-time mission reveal animation'); assertIncludes(boardSource, 'island-run-board--diplomatic-activation', 'Tiles should receive the activation pulse class'); } },
  { name: 'every landmark camera target comes from its fixed manifest plot', run: () => { assertIncludes(boardSource, 'const landmarkFocusPlacement = landmark?.levelZeroPlacement ?? landmark;', 'Landmark focus should prefer the persistent plot center'); assertIncludes(boardSource, 'insetLandmarkCameraFocus(', 'Edge landmarks should use the phone-safe focus inset'); assertIncludes(boardSource, 'focusX: focusPosition.x', 'Orbit visuals should carry manifest focus X'); assertIncludes(boardSource, 'focusY: focusPosition.y', 'Orbit visuals should carry manifest focus Y'); assertIncludes(boardSource, 'visual.focusX ?? visual.x, visual.focusY ?? visual.y', 'Camera focus should use manifest plot coordinates'); assert(!boardSource.includes("setActiveStopId(null);\n                setCameraMode('board_follow');\n                setLandingText"), 'Closing the ticket modal should preserve stop focus until a roll resumes board-follow'); } },
  ...
  { name: 'queue priority puts B30 before ambient B24 and after B29', run: () => assertIncludes(hookSource, "'I001-B29': 4, 'I001-B30': 5, 'I001-B24': 6", 'Expected B30 to wait for B29 and outrank ambient B24') },

  { name: 'B29 receives no reward callback', run: () => { assert(!boardSource.includes('onRewardClaim='), 'StoryReader episodes must not receive a gameplay reward callback'); assertIncludes(hookSource, "setActiveStoryEpisode({ kind: 'island_resolution', manifestPath: ISLAND_001_RESOLUTION_MANIFEST_PATH })", 'Resolution should use existing StoryReader episode state'); } },
  ...
  { name: 'queue priority keeps B29 before ambient B24', run: () => assertIncludes(hookSource, "'I001-B29': 4, 'I001-B30': 5, 'I001-B24': 6", 'Expected queue priority') },
  { name: 'board uses episode-specific StoryReader state without a global prologue', run: () => { assertIncludes(boardSource, 'ActiveIslandStoryEpisode', 'Board should use episode state'); assert(!boardSource.includes("kind: 'global_prologue'"), 'Retired global prologue must not remain in board state'); assert(!boardSource.includes('openGlobalStoryReader'), 'Concord must not replay the retired global story'); assertIncludes(hookSource, "kind: 'island_arrival'", 'Island arrival episode should remain available for later playback'); assertIncludes(hookSource, "kind: 'island_resolution'", 'Island resolution episode should be explicit'); } },
  { name: 'arrival and resolution receive no reward callback', run: () => assert(!boardSource.includes('onRewardClaim='), 'Narrative StoryReader should be presentation-only') },
  ...
  { name: 'dialogue copy renders through reusable component', run: () => { assertIncludes(boardSource, '<IslandNarrativeDialogue', 'Expected reusable dialogue component'); assertIncludes(definitionSource, 'Start small. Wake one gentle place — then ask why it was ever put to sleep.', 'Expected Miri copy'); assertIncludes(definitionSource, 'The first cradle is listening. Stay close while the Hatchery wakes.', 'Expected Poko copy'); } },
