...
export const islandNarrativeOpeningFlowTests: TestCase[] = [
  ...
  { name: 'diplomatic reward channel is hidden only before first Island 1 Hatchery restoration', run: () => { assert(!isDiplomaticRewardChannelVisible({ currentIslandNumber: 1, cycleIndex: 0, hatcheryBuildLevel: 0 }), 'Fresh Island 1 should hide reward channel'); assert(isDiplomaticRewardChannelVisible({ currentIslandNumber: 1, cycleIndex: 0, hatcheryBuildLevel: 1 }), 'Hatchery L1 should reveal reward channel'); assert(isDiplomaticRewardChannelVisible({ currentIslandNumber: 2, cycleIndex: 0, hatcheryBuildLevel: 0 }), 'Later islands should show reward channel'); assert(isDiplomaticRewardChannelVisible({ currentIslandNumber: 1, cycleIndex: 1, hatcheryBuildLevel: 0 }), 'Repeat cycle should show reward channel'); } },
  { name: 'diplomatic reveal never hides the core reward bar or its side rails', run: () => { assertIncludes(boardSource, 'const diplomaticRewardChannelVisible = isDiplomaticRewardChannelVisible({', 'Board should derive presentation gate for diplomatic-only surfaces'); assertIncludes(boardSource, 'diplomaticRewardChannelVisible && ISLAND_RUN_CONTRACT_V2_ENABLED', 'Compact top reward channel may remain gated'); assertIncludes(boardSource, '<div className="island-run-board__rewardbar-cluster">', 'Core reward bar should always render'); assert(!boardSource.includes('{diplomaticRewardChannelVisible ? ('), 'Diplomatic reveal must not gate the core reward bar cluster'); assertIncludes(boardSource, 'shouldShowBestNextActionChip && bestNextAction && (', 'Reward-bar claim guidance should remain available'); } },
