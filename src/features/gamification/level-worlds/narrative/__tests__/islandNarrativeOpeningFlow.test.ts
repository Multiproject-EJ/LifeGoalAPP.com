...
export const islandNarrativeOpeningFlowTests: TestCase[] = [
  ...
  { name: 'board gates reward-channel presentation through the diplomatic helper', run: () => { assertIncludes(boardSource, 'const diplomaticRewardChannelVisible = isDiplomaticRewardChannelVisible({', 'Board should derive presentation gate'); assertIncludes(boardSource, 'diplomaticRewardChannelVisible && ISLAND_RUN_CONTRACT_V2_ENABLED', 'Top reward channel should be gated'); assertIncludes(boardSource, '{diplomaticRewardChannelVisible ? (', 'Board reward bar should be gated'); } },
