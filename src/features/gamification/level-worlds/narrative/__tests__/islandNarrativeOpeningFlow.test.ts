...
export const islandNarrativeOpeningFlowTests: TestCase[] = [
  ...
  { name: 'only opening beats plus B24 are queued/displayed', run: () => { assertIncludes(hookSource, "type OpeningBeatId = 'I001-B02' | 'I001-B03' | 'I001-B04'", 'Opening hook should preserve opening beat IDs'); assertIncludes(hookSource, "type AmbientBeatId = 'I001-B24'", 'B24 should be the only ambient beat implemented'); ['I001-B26', 'I001-B29', 'I001-B30'].forEach((id) => assert(!hookSource.includes(`'${id}'`), `This PR must not implement ${id}`)); } },
  ...
  { name: 'queue priority puts arrival before dialogue', run: () => assertIncludes(hookSource, "const QUEUE_PRIORITY: Record<IslandNarrativeControllerBeatId, number> = { 'I001-B02': 0, 'I001-B03': 1, 'I001-B04': 2, 'I001-B24': 3 }", 'Expected queue priority') },
