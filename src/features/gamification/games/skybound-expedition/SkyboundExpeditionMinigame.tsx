...
import {
  ...
  getSkyboundFlightScoreBreakdown, getSkyboundLevel, getSkyboundTouchdownResult, scoreSkyboundFlight, stepSkyboundFlight, upgradeSkyboundPart,
  type SkyboundFlightControl, type SkyboundFlightState, type SkyboundStuntKind, type SkyboundUpgradeKind, type SkyboundUpgrades,
...
import { startSkyboundEngineAudio, type SkyboundEngineAudioController } from './skyboundFlightAudio';
...
const STUNT_COPY:Record<Exclude<SkyboundStuntKind,null>,{title:string;detail:string}>={
  near_miss:{title:'NEAR MISS',detail:'Hazard proximity held without contact'},
  terrain_skim:{title:'TERRAIN SKIM',detail:'Low, fast, and clear of the ground'},
  barrel_roll:{title:'BARREL ROLL',detail:'A complete high-energy rotation'},
  crash_finale:{title:'CRASH FINALE',detail:'The recovery crew scored the spectacle'},
};
...
export default function SkyboundExpeditionMinigame({onComplete,ticketBudget=0,launchConfig,evaluatorConfig}:SkyboundExpeditionMinigameProps) {
  ...
  const stuntSerialRef=useRef(0);
  const flowLockedRef=useRef(false);
  const launchChargeRef=useRef(false);
  const engineAudioRef=useRef<SkyboundEngineAudioController|null>(null);
  ...
  const [stuntPulse,setStuntPulse]=useState<{serial:number;kind:Exclude<SkyboundStuntKind,null>;bonus:number}|null>(null);
  ...
  const courseProfile=lesson.standards.some((standard)=>standard.kind==='landing')
    ? 'landing' as const
    : lesson.id==='ace_formation'
      ? 'gold_formation' as const
      : lesson.id==='elite_storm'||lesson.id==='elite_exam'
        ? 'storm_corridor' as const
        : 'standard' as const;
  const courseProfileLabel=courseProfile==='gold_formation'
    ? 'GOLD WINGS FORMATION'
    : courseProfile==='storm_corridor'
      ? 'AUTHORED STORM CORRIDOR'
      : courseProfile==='landing'
        ? 'LANDING PATTERN'
        : level.trainingStage;
  ...
  const launch=useCallback(async()=>{
    ...
    engineAudioRef.current??=startSkyboundEngineAudio(rank.aircraftId,Boolean(evaluatorConfig));
  ...
  },[canonicalConfig,courseProfile,evaluatorConfig,lesson.goalDistance,lesson.id,lesson.levelId,level.id,rank.aircraftId,setAcademy,setAim,setPhase,setUpgrades]);
  ...
  useEffect(()=>{
    const serial=flight?.stuntSerial??0;
    const kind=flight?.lastStuntKind??null;
    if(phase!=='flying'||serial<=stuntSerialRef.current||!kind)return undefined;
    stuntSerialRef.current=serial;
    const bonus=flight?.lastStuntBonus??0;
    const copy=STUNT_COPY[kind];
    setStuntPulse({serial,kind,bonus});
    setMessage(`${copy.title} +${bonus} — ${copy.detail.toLowerCase()}.`);
    playIslandRunSound('reward_bar_fill');
    const timeout=window.setTimeout(()=>setStuntPulse(null),980);
    return()=>window.clearTimeout(timeout);
  },[flight?.lastStuntBonus,flight?.lastStuntKind,flight?.stuntSerial,phase]);
  ...
  useEffect(()=>{
    ...
    const telemetryState=getSkyboundFlightTelemetry(flight);
    engineAudioRef.current?.update(flight,controlRef.current.boost,telemetryState.condition==='stall'||telemetryState.condition==='damaged'||telemetryState.condition==='ground-effect');
  },[flight,phase]);

  useEffect(()=>()=>{engineAudioRef.current?.stop();engineAudioRef.current=null;},[]);
  ...
  const prepareSortie=(nextLessonId:SkyboundLessonId)=>{const nextLesson=getSkyboundLesson(nextLessonId);engineAudioRef.current?.stop();engineAudioRef.current=null;setSelectedRankId(nextLesson.rankId);setLessonId(nextLessonId);flightRef.current=null;setFlight(null);setEvaluation(null);controlRef.current={...DEFAULT_CONTROL};flightDragRef.current=null;setFlightStick(null);impactSerialRef.current=0;stuntSerialRef.current=0;flowLockedRef.current=false;setImpactPulse(false);setFlowPulse(false);setStuntPulse(null);launchChargeRef.current=false;setLaunchCharging(false);setAim(DEFAULT_AIM);setSortieKey((value)=>value+1);setMessage(`${getSkyboundRank(nextLesson.rankId).launchMethod}: pull down, release, then fly.`);setPhase('aiming');};
  ...
  const touchdownResult=flight?getSkyboundTouchdownResult(flight):null;
  ...
  return <main className={`skybound skybound--${rank.id}`} aria-label="Skybound Pilot Academy full 3D event">
    <section className="skybound__shell">
      ...
      <div className="skybound__course-row"><div><strong>{rank.callsign} · {lesson.name}</strong><span>{lesson.briefing}</span><em>{courseProfileLabel} · AGL {level.targetAltitudeMin}–{level.targetAltitudeMax}m</em></div><strong>{distance} / {lesson.goalDistance}m</strong></div>
