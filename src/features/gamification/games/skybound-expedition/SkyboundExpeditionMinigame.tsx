...
import {
  ...
  getSkyboundFlightScoreBreakdown, scoreSkyboundFlight, stepSkyboundFlight, upgradeSkyboundPart,
...
import {
  ...
  getSkyboundRank, getSkyboundRankLessons, installSkyboundNextAssemblyPart, isSkyboundLessonUnlocked, settleSkyboundAcademyLesson,
...
import { clearSkyboundAcademySave, loadSkyboundAcademySave, saveSkyboundAcademySave, type SkyboundAcademyEventProgress } from '../../level-worlds/services/skyboundAcademyStorage';
...
export default function SkyboundExpeditionMinigame({onComplete,ticketBudget=0,launchConfig}:IslandRunMinigameProps) {
  const canonicalConfig=launchConfig?.mode==='pilot_academy'?launchConfig as unknown as SkyboundCanonicalLaunchConfig:null;
  const initial=useRef(canonicalConfig?.initialProgress??loadSkyboundAcademySave()).current;
  ...
  const launch=useCallback(async()=>{
    ...
    const next=createSkyboundFlight({power:currentAim.power,angleDeg:currentAim.angleDeg,upgrades:upgradesRef.current,levelId:lesson.levelId,goalDistance:lesson.goalDistance,aircraftId:rank.aircraftId,assemblyLevel:getSkyboundAssemblyLevel(academyRef.current,rank.id)});
  ...
  const fullPowerLaunch=()=>{
    ...
    const chargeSteps=[
      ...
      {power:1,angleDeg:34,pullX:-20,pullY:150,dragging:true},
    ];
  ...
  return <main className={`skybound skybound--${rank.id}`} aria-label="Skybound Pilot Academy full 3D event">
    <section className="skybound__shell">
      ...
      <div className={`skybound__stage is-${phase}${telemetry?` is-${telemetry.condition}`:''}${impactPulse?' has-impact':''}`}>
        <SkyboundExpeditionThreeStage phase={phase} levelId={lesson.levelId} goalDistance={lesson.goalDistance} aircraftId={rank.aircraftId} assemblyLevel={assemblyLevel} flight={flight} aim={aim} sortieKey={sortieKey} boosting={controlRef.current.boost} stabilizing={controlRef.current.stabilize===true} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}/>
