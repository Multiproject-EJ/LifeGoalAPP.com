import type { MinerLayoutVersion } from '../../level-worlds/services/crystalMinersCourses';
import { minerCourseHeight,  minerBlockOre } from '../../level-worlds/services/crystalMinersGame';
...
import { createMinerVisualEvents, minerVisibleEffects, createMinerReplayTiming, minerReplayFrameAt, createMinerReplayCamera } from '../../level-worlds/services/crystalMinersReplay';
...
const PALETTES: Record<MinerBlock['kind'], [string, string, string]> = {
  charger:['#247c87','#a9ffff','#15414f'],key:['#b99148','#fff1a8','#695039'],gate:['#4a5478','#c2b0e3','#232c46'],
...
function CaveScenery({level,height=MINER_HEIGHT}:{level:number;height?:number}) {
  const theme=getMinerTheme(level);
  return <g opacity=".55" aria-hidden="true">{Array.from({length:Math.ceil(height/150)},(_,i)=>{
...
function MineScene({ blocks, frame, digging, height=MINER_HEIGHT, layoutVersion=10, visualHits, survey = 'top', camera, level, freshChests = [] }: { height?:number; layoutVersion?:MinerLayoutVersion; visualHits?:import('../../level-worlds/services/crystalMinersGame').MinerHit[]; freshChests?:number[]; level: number; camera?: { y: number; toolId: number | null; returning: boolean }; blocks: MinerBlock[]; frame: MinerFrame | null; digging: boolean; survey?: 'top' | 'all' | 'bottom' | 'guardian' }) {
  const theme=getMinerTheme(level);
  const cameraY = frame ? camera?.y ?? 0 : survey === 'bottom' ? height - 386 : survey === 'guardian' ? MINER_BLOCK_TOP + 8*MINER_BLOCK_HEIGHT - 80 : 0;
  const liveHits = frame?.hits ?? [];
  const recentOre = liveHits.filter(h=>h.broken).reduce((sum,h)=>sum+minerBlockOre({id:h.blockId,kind:h.kind},level,layoutVersion),0);
  return <svg className="cm-mine" viewBox={`-22 ${cameraY} 344 ${!digging && survey === 'all' ? height : 386}`} role="img" aria-label={digging ? 'Tools bouncing through the mine' : 'Layered mine with five finish-line treasure chests'}>
    <defs>
    ...
    <rect x="-22" width="344" height={height} rx="14" fill="url(#cm-cave)" />
    <ellipse cx="150" cy={cameraY+180} rx="180" ry="260" fill="url(#cm-glow)"/>
    <CaveScenery level={level} height={height}/>
    <path d={`M-10 ${height}V25Q-10 12 2 12H298Q310 12 310 25V${height}`} stroke="#846d50" strokeWidth="8" fill="none" />
    <path d="M-10 47h320M-10 150h9m302 0h9M-10 260h9m302 0h9" stroke="#c0a578" strokeWidth="4" />
    {[0,5,10,15,20,25].filter(row=>MINER_BLOCK_TOP+row*MINER_BLOCK_HEIGHT<height-100).map(row=><text key={row} x="-17" y={MINER_BLOCK_TOP + row*MINER_BLOCK_HEIGHT+10} fill="#9abeb5" fontSize="7" textAnchor="middle">{row}</text>)}
    {[0,1,2,3,4].map(i => <g key={i} opacity=".22"><path d={`M${i*60+30} 20v${height-70}`} stroke="#a5d3d1" strokeDasharray="2 9"/><circle cx={i*60+30} cy="23" r="3" fill="#bbded0"/></g>)}
    {blocks.map(block => {
      const x = block.id % 5 * 60 + 1; const y = minerBlockY(block,level,layoutVersion);
    ...
    {(visualHits??frame?.hits??[]).map(hit=><MinerBlockImpact key={`${hit.step}-${hit.blockId}`} hit={hit} level={level}/>)}
    ...
    <text x="150" y={height-18} textAnchor="middle" fill="#85a7a8" fontSize="9" letterSpacing="3">THE CRYSTAL DEPTHS</text>
  </svg>;
}
...
function CrystalMinersGame({ bridge, onComplete, requestResources, ticketOffersEnabled = false, inspectEnabled = false }: { inspectEnabled?:boolean; bridge: CrystalMinersBridge; requestResources?: (destination: MinerResourceDestination)=>void; ticketOffersEnabled?: boolean; onComplete: IslandRunMinigameProps['onComplete'] }) {
  ...
  const [effectClock,setEffectClock]=useState(0);
  const [replayLayout,setReplayLayout]=useState<MinerLayoutVersion>(10);
  ...
  const replayCamera = useMemo(()=>replay ? createMinerReplayCamera(replay.frames,replay.courseHeight) : [],[replay]);
  const replayTiming=useMemo(()=>replay?createMinerReplayTiming(replay.frames):null,[replay]);
  const visualEvents=useMemo(()=>replay&&replayTiming?createMinerVisualEvents(replay.frames,replayTiming.times):[],[replay,replayTiming]);
  const visualHits=minerVisibleEffects(visualEvents,effectClock,replayCamera[frameIndex]?.y??0);
  ...
  const courseHeight=replay?.courseHeight??minerCourseHeight(visibleLevel,progress.layoutVersion??10);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      setFrameIndex(index);
      setEffectClock(reduced?Number.POSITIVE_INFINITY:elapsed);
  ...
  const act = async (command: MinerCommand) => {
    ...
    try {
      ...
      if (result.simulation) { setReplayLayout(progress.layoutVersion??10);setEffectClock(0);skipReplay.current=false;setChestCelebration(false);setMergeHover(null);flownTickets.current.clear();flightTimers.current.forEach(clearTimeout);flightTimers.current=[];setRevealedTickets(0);setTicketFlights([]);setMobileView('mine');setClaimNotice(''); setSurvey('top'); setFrameIndex(0); setReplay(result.simulation); setResultVisible(false); }
    ...
    <div className="cm-shell" aria-hidden={help || leagueOpen || resultVisible || undefined}>
      ...
      <div className="cm-game-layout" data-mobile-view={replay?'mine':inspectEnabled?mobileView:'workshop'}>
        <section className="cm-workshop">
          ...
          <div className="cm-path-preview" aria-label="Five path previews">{[0,1,2,3,4].map(lane=>{const special=progress.blocks.find(b=>b.hp>0&&b.id%5===lane&&['key','gate','charger','spawner','ticket','deflector','tnt'].includes(b.kind));const names:Record<string,string>={key:'Runic key',gate:'Linked gate',charger:'Tool charger',spawner:'Bonus pick',ticket:'Extra drop',deflector:'One-use turn',tnt:'Demolition'};const icons:Record<string,string>={key:'⚿',gate:'▥',charger:'ϟ',spawner:'✚',ticket:'+1',deflector:'↪',tnt:'✹'};const equipped=readiness.laneCapacity[lane]>=readiness.laneResistance[lane]*1.15&&readiness.laneCapacity[lane]>0;return <button type="button" key={lane} data-equipped={equipped} disabled={Boolean(replay)} onClick={()=>setMessage(`Path ${lane+1}: ${special?names[special.kind]:'treasure route'}. ${equipped?'Estimated capacity ready':'More digging capacity suggested'}. Aim for tier ${minerRecommendedTier(progress.level)}+ tools.`)} aria-label={`Path ${lane+1}: ${special?names[special.kind]:'treasure'}`}><span>{lane+1}</span><b>{special?icons[special.kind]:'◇'}</b></button>;})}</div>
        ...
        <section className="cm-expedition">
          ...
          {inspectEnabled&&!replay && !campaignComplete && <div className="cm-readiness"><strong>{readiness.stage === 'boss' ? 'Guardian readiness' : readiness.stage === 'approach' ? 'Hard approach readiness' : 'Fleet readiness'}</strong><span>{readiness.readyLanes} paths equipped · {readiness.neededLanes} needed</span><small>{readiness.readyLanes < readiness.neededLanes ? 'Merge, reposition or upgrade before spending a ticket. Damage stays saved between digs.' : 'Estimated digging capacity reached. Gates, bonus picks and guardian attacks still affect the result.'}</small></div>}
          ...
          <div className="cm-mine-frame"><MineScene height={courseHeight} layoutVersion={replay?replayLayout:progress.layoutVersion??10} visualHits={visualHits} freshChests={freshChests} level={visibleLevel} blocks={frame?.blocks ?? progress.blocks} frame={frame} digging={Boolean(replay)} survey={survey} camera={chestCelebration?{y:courseHeight-386,toolId:null,returning:false}:replayCamera[frameIndex]}/>
