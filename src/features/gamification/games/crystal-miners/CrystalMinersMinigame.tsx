...
function GuardianArt({block,level,hit}:{block:MinerBlock;level:number;hit:boolean}) {
  ...
  return <g className={`cm-boss cm-boss-stage-${stage+1}${hit?' is-hit':''}${enraged?' is-enraged':''}`} style={{'--boss-accent':style.accent,'--boss-speed':`${2.5-stage*.35}s`} as CSSProperties}>
...
function MineScene({ blocks, frame, digging, survey = 'top', camera, level }: { level: number; camera?: { y: number; toolId: number | null; returning: boolean }; blocks: MinerBlock[]; frame: MinerFrame | null; digging: boolean; survey?: 'top' | 'all' | 'bottom' | 'guardian' }) {
  ...
  return <svg className="cm-mine" viewBox={`-22 ${cameraY} 344 ${!digging && survey === 'all' ? MINER_HEIGHT : 386}`} role="img" aria-label={digging ? 'Tools bouncing through the mine' : 'Layered mine with five finish-line treasure chests'}>
    <defs>
      <linearGradient id="cm-cave" x2="0" y2="1"><stop stopColor="#152f3d"/><stop offset="1" stopColor="#07161f"/></linearGradient>
      <radialGradient id="cm-glow"><stop stopColor="#72ebbd" stopOpacity=".35"/><stop offset="1" stopColor="#72ebbd" stopOpacity="0"/></radialGradient>
    </defs>
    ...
    {blocks.map(block => {
      ...
      if (block.kind === 'treasure') {
        ...
        return <g key={block.id} transform={`translate(${x},${y-9})`} className={approaching ? 'cm-chest-approach' : block.hp === 0 ? 'cm-chest-revealed' : ''}><ellipse className="cm-chest-halo" cx="29" cy="25" rx="29" ry="28" fill={prize.color} opacity={approaching ? '.6' : '.15'}/>{approaching && <path d="m29-14 3 11 10-3-7 8 7 8-10-3-3 11-3-11-10 3 7-8-7-8 10 3z" fill="#fff2ab"/>}<svg width="58" height="46" viewBox="0 0 60 48"><TreasureChest opened={block.hp === 0}/></svg>{block.hp === 0 && <><text className="cm-chest-prize" x="29" y="-10" textAnchor="middle" fill="#fff3b3" fontSize="8" fontWeight="800">{prize.gifts ? '🎁 +25 ore' : `+${prize.ore} ore`}</text><path d="M8-7 5-15m45 8 4-8M29-20v-8" stroke="#fff3b3" strokeWidth="2"/></>}</g>;
      }
      if (block.hp === 0) return null;
      if (block.kind === 'boss') return <g key={block.id} transform={`translate(0,${y})`}><GuardianArt block={block} level={level} hit={Boolean(frame?.hits.some(h=>h.blockId===block.id))}/></g>;
      const [face, top, edge] = PALETTES[block.kind];
...
function CrystalMinersGame({ bridge, onComplete, requestResources, ticketOffersEnabled = false }: { bridge: CrystalMinersBridge; requestResources?: (destination: MinerResourceDestination)=>void; ticketOffersEnabled?: boolean; onComplete: IslandRunMinigameProps['onComplete'] }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      else { bridge.observe('result_shown'); setReplay(null); setResultVisible(true); if (!mutedRef.current) playIslandRunSound('sticker_complete'); triggerIslandRunHaptic('sticker_complete'); }
    };
  ...
  return <div ref={root} tabIndex={-1} className={`cm-overlay${replay ? ' cm-is-running' : ''}`} role="dialog" aria-modal="true" aria-labelledby="cm-title"
    ...
    <div className="cm-shell" aria-hidden={help || leagueOpen || resultVisible || undefined}>
      ...
      <section className="cm-event-track" aria-label="Cavern completion rewards">
        ...
        <div className="cm-track-prizes">{MINER_EVENT_MILESTONES.map(prize=>{
        ...
        <section className="cm-workshop">
          ...
          <div className={`cm-tool-rack${replay ? ' is-mining' : ''}`} aria-label="Tool rack. Select a tool, then a matching tool to merge, or an empty slot to move.">
            {progress.tools.map((tier,index)=><button type="button" data-miner-slot={index} key={index}
              ...
              onPointerUp={e=>{
                const start = drag.current; drag.current = null; setDragPoint(null);
          ...
          <p className="cm-hint" role="status">{message || (selected !== null ? `Tier ${progress.tools[selected]} selected. Tap a matching tool or a new slot.` : 'Drag matching tools together, or tap one then the other.')}</p>
          <div className="cm-workshop-note"><span>✧</span><p>Small steps. Deeper discoveries.<small>Your tools and upgrades travel with you to every island.</small></p></div>
        </section>
        <section className="cm-expedition">
          <div className="cm-section-heading"><div><p className="cm-eyebrow">{replay ? 'EXPEDITION IN PROGRESS' : 'EXPLORE THE DEPTHS'}</p><h2>Cavern {visibleLevel.toString().padStart(2,'0')} / 40</h2></div><span className="cm-depth">{isMinerRewardCavern(visibleLevel) ? 'BOSS + REWARDS · ' : isMinerBossCavern(visibleLevel) ? 'BOSS · ' : ''}{(visibleLevel-1)*MINER_ROWS}—{visibleLevel*MINER_ROWS} m</span></div>
          {!replay && <p className="cm-course-hint">{campaignComplete ? 'All 40 caverns explored. Your workshop is saved.' : isMinerRewardCavern(visibleLevel) ? `Defeat the guardian, then enjoy the treasure fall! Aim for tier ${minerRecommendedTier(visibleLevel)}+ tools.` : `Aim for tier ${minerRecommendedTier(visibleLevel)}+ tools · ${requiredChests === 2 ? 'Hard approach · reach 2 chests to pass' : 'Reach any chest · tools stay in their lane'}`}</p>}
