  ...
  defaultState.journalEntries = [
    {
      ...
      content:
        'I have no doubt this launch will be perfect and I am absolutely certain the plan cannot be wrong.',
      mood: 'confident',
    ...
    },
    {
      ...
      content:
        'This has to work, no matter what. It feels like the only way forward and I cannot fail.',
      mood: 'anxious',
      tags: ['pressure', 'focus'],
      is_private: true,
      attachments: null,
...
export function addDemoGoal(payload: GoalInsert): GoalRow {
  const record = normalizeGoalRow({
    ...
    secondary_life_wheel_categories: payload.secondary_life_wheel_categories ?? [],
...
export function upsertDemoHabit(payload: HabitInsert | HabitUpdate): HabitRow {
  ...
  updateState((current) => {
    ...
    nextRecord = {
      ...
      status: payload.status ?? (payload as HabitInsert).status ?? ((payload.archived ?? (payload as HabitInsert).archived ?? false) ? 'archived' : 'active'),
      ...
      paused_at: payload.paused_at ?? (payload as HabitInsert).paused_at ?? null,
      paused_reason: payload.paused_reason ?? (payload as HabitInsert).paused_reason ?? null,
      resume_on: payload.resume_on ?? (payload as HabitInsert).resume_on ?? null,
      deactivated_at: payload.deactivated_at ?? (payload as HabitInsert).deactivated_at ?? null,
      deactivated_reason: payload.deactivated_reason ?? (payload as HabitInsert).deactivated_reason ?? null,
 ...
 *
 * All four notification_type values from ScheduledReminder are represented:
 * - 'habit_reminder'  — per-habit morning/daily reminders
 * - 'streak_warning'  — alert when a habit streak is at risk
 * - 'checkin_nudge'   — evening life wheel check-in prompt (18:00)
 * - 'coach_nudge'     — nightly Game of Life Coach nudge (20:30)
 */
...
export type DemoTreatHatch = {
  ...
  numbers: null;
  number_reward: null;
  symbol_reward: string;
  reward_payload: { reward_type: string; reward_value: number };
  created_at: string;
};

export type DemoTreatSeason = {
  id: string;
  theme_name: string;
  starts_on: string;
  ends_on: string;
...
export type DemoTreatCalendarData = {
  season: DemoTreatSeason;
  hatches: DemoTreatHatch[];
  progress: DemoTreatProgress;
