...
export interface MinerLevelMetrics {level:number;attempts:number;clears:number;ore:number;chests:number;forge:number;highestTier:number;}
/** Interpret only versioned attempt summaries; lifecycle/blocked/error events cannot inflate play counts. */
export function summarizeMinerTelemetry(events: RecentTelemetryEventRow[]) {
  ...
  for(const row of events){
    const m=row.metadata??{};if(m.game_id!=='crystal_miners')continue;
    ...
    const entry=levels.get(level)??{level,attempts:0,clears:0,ore:0,chests:0,forge:0,highestTier:0};
    entry.attempts++;entry.clears+=m.outcome==='cleared'?1:0;entry.ore+=n('ore_gained');entry.chests+=n('new_chests');entry.forge+=n('forge_before');entry.highestTier+=n('highest_tier');levels.set(level,entry);
  }
